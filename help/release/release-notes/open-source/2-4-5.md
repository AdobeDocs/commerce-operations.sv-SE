...
* Nu kan du byta namn på en datakorrigering och lägga till det gamla klassnamnet som alias i databastabellen `patch_list`. Magento Open Source kontrollerar nu om det redan finns aliaser för datakorrigering i databasen innan korrigeringen tillämpas. Tidigare uppstod ett fel under dessa förhållanden i applicationOpen Source.
...
* Cacheminnet för en hel sida visas inte längre som inaktiverat i Admin när cacheminnet för Magento Open Source har tömts och `use_stale_cache` har aktiverats.
...
* `cart`-frågan returnerar inte längre null-svar när en produkt inte finns i lager. Ett nytt `errors`-element som innehåller felmeddelandet introducerades i svaret. Tidigare visade programmet Open Source ett `null`-värde under avsnittet `items` i svaret när du körde en fråga med en produkt som inte fanns i lager. Se [kundvagnsfråga](https://devdocs.magento.com/guides/v2.4/graphql/queries/cart.html).
...
* I Magento Open Source visas nu ett felmeddelande när du anger en ogiltig cookie-domän (**Store** > **Configurations** > **Web** > **Cookie-standardinställningsdomän**). Tidigare kraschade webbplatsen. [GitHub-35048](https://github.com/magento/magento2/issues/35048)
...
* Magento Open Source-sessionerna avslutas inte längre efter det att en GraphQL-förfrågan har gjorts. Tidigare loggade plugin-programmet `ClearCustomerSessionAfterRequest` ut till shopparen. [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* På sidan **Butik** > **Konfiguration** > **Tjänster** visas nu Magento Open Source Web API-information som förväntat när Resursåtkomst har angetts till **Anpassad** på fliken Rollresurser. [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* Magento Open Source visar nu ett korrekt antal förslag på sökresultat i butiken i distributioner där sökförslag och inställningen **Visa resultatantal för varje förslag** är aktiverad. Tidigare var antalet som visades bredvid nyckelorden noll.
