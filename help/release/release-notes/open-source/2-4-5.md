...
* Nu kan du byta namn på en datakorrigering och lägga till det gamla klassnamnet som ett alias i `patch_list` databastabell. Magento Open Source kontrollerar nu om det redan finns aliaser för datakorrigering i databasen innan korrigeringen tillämpas. Tidigare genererades ett fel i applicationOpen Source under dessa förhållanden.
...
* The `cart` -frågan returnerar inte längre null-svar när en produkt inte finns i lager. En ny `errors` -element som innehåller felmeddelandet lades till i svaret. När du tidigare körde en fråga med en produkt som inte finns i lager visades en `null` värdet under `items` i svaret. Se [kundvagnsfråga](https://devdocs.magento.com/guides/v2.4/graphql/queries/cart.html).
...
* Kundvagnens innehåll och inloggningsstatus läses nu in på nytt som förväntat efter att en session har avslutats när **Aktivera beständighet** är inställd på **Ja**. [GitHub-35182](https://github.com/magento/magento2/issues/35182)
...
* Du kan nu ändra produktgränsen per sida som visas i en kategori när **Kom ihåg kategorisidnumrering**  är aktiverat. Tidigare fanns det en cookie `form_key` och användargränssnitt `form_key` och Magento Open Source visade det här felet:  `Invalid Form Key. Please refresh the page`.
...
* Magento Open Source returnerar nu felet 404 i stället för 500 vid navigering till `/checkout/sidebar/updateItemQty/?item_qty=error` på butiken. Tidigare utlöstes detta fel:  `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`. [GitHub-34380](https://github.com/magento/magento2/issues/34380)
...
* Merchants kan nu lägga till ett skiktprisattribut (`tier_price`) till produktjämförelser. Tidigare kraschade sidan för produktjämförelser när **Jämförelsebar på butiken** inställning för det här attributet har aktiverats. [GitHub-35244](https://github.com/magento/magento2/issues/35244)
...
* Borttagen som inte behövs `csp_whitelist.xml` filer. [GitHub-30607](https://github.com/magento/magento2/issues/30607)
...
* The `generateCustomerToken` skapar nu en post i `customer_log` som förväntat efter generering av en kundtoken. [GitHub-33378](https://github.com/magento/magento2/issues/33378)
...
* REST API-satsbegäranden i PUT och DELETE fungerar nu som väntat när `Magento_ReCaptchaWebapiRest` är aktiverad. [GitHub-35348](https://github.com/magento/magento2/issues/35348)
