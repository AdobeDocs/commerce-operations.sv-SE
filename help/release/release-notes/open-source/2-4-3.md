...
Magento Open Source 2.4.3 har förbättrat prestanda och säkerhet samt gjort betydande plattformsförbättringar. Säkerhetsförbättringarna omfattar bland annat utvidgning av reCAPTCHA-täckning och inkludering av inbyggd hastighetsbegränsning. Beroenden för Core Composer och tredjepartsbibliotek har uppgraderats till de senaste versionerna som är kompatibla med PHP 8.x. Page Builder finns nu som pakettillägg i Magento Open Source. Det är nu standardverktyget för innehållsredigering för Adobe Commerce och Magento Open Source.
...
* The `updateProductsInWishlist` mutationen uppdaterar nu objekt som tillhör en paketprodukt i en önskelista. I stället för att uppdatera önskelisteobjektet tog den här mutationen bort objektet och skapade ett nytt, som ändrade objekt-ID.
...
* Insticksprogram för Global Magento Open Source (till exempel `webapi_rest` och `graphql` ) aktiveras inte längre för en ny anpassad områdestyp när `di.xml` i det här området innehåller inga registrerade plugin-program.
...
* Prestanda för  `catalog_product_alert` Processen vid körning av stora tabeller (flera miljoner rader) har förbättrats. Tidigare `catalog_product_alert` har läst in alla produktvarningar, vilket orsakade ett undantagsfel som inte fanns i minnet.
...
* Programmet avmarkerar inte längre **Lägg till kommentarer** kryssruta när en kund klickar **Få leveransmetoder och fraktpriser** och väljer en leveransmetod när du skapar en beställning från administratören.
...
* Ett test har lagts till för det här scenariot: Administratörsanvändare kan redigera ett kundkonto när kunden prenumererar på ett nyhetsbrev i kö.
