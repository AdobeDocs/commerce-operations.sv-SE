...
Se [Versionsinformation om Adobe Commerce 2.4.2-p2](../security/2-4-2-patches.md) för information om Adobe Commerce 2.4.2-p2.
...
* **Önsklistorna**. The [addWishlistItemsToCart-mutation](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-wishlist-items-to-cart.html) flyttar artiklar från den angivna önskelistan till kundvagnen. <!-- magento/partners-magento2ee#449-->
...
* Om du kompilerar färre filer med Grunde eller med kompilering på serversidan får du nu samma resultat. Tidigare. `.abs- styles`, som utökar andra `.abs- styles` in `_extends.less`, kunde inte skapas korrekt vid kompilering med Grunt. Detta resulterade i skillnader mellan produktions- och utvecklingsdistributioner. [GitHub-7231](https://github.com/magento/magento2/issues/7231)
...
* Alla kömeddelanden för konsument `quoteItemCleaner` ändra status till `complete` som förväntat efter att flera produkter har tagits bort. Tidigare ändrade bara ett meddelande för den här konsumenten sin status till `complete`och resten har ändrat status till `in progress`.
...
* Programmet genererar inte längre något fel när en administratör med begränsad behörighet lägger till en produktwidget på en CMS-sida i Admin. Tidigare inträffade det här felet när administratören klickade på **Spara** knapp:  `We are sorry, an error has occurred while generating the content`.
...
* Globala Adobe Commerce-plugin-program (till exempel `webapi_rest` och `graphql` ) aktiveras inte längre för en ny anpassad områdestyp när `di.xml` i det här området innehåller inga registrerade plugin-program.
...
* Filtrering fungerar nu som väntat i listan över schemalagda exporter när entitetstypen har valts. Tidigare fungerade inte filtret och ett JavaScript-fel uppstod i programmet. [GitHub-361](https://github.com/magento/partners-magento2ee/issues/361)
...
* **Händelsedatum** värdena är nu desamma i butiken och Admin när presentregistret skapas eller redigeras. Varje gång du redigerade och sparade presentregistret justerades **Händelsedatum** en dag bakåt på både butiken och Admin. [GitHub-466](https://github.com/magento/partners-magento2ee/issues/466)
...
* Programmet genererar inte längre något fel när en administratör försöker läsa in en sida på butiken när Google Analytics är aktiverat. En CSP-post för `connect-src` har lagts till för att tillåta AJAX förfrågningar to www.google-analytics.com. [GitHub-30880](https://github.com/magento/magento2/issues/30880)
...
* Nu kan man använda GraphQL `setShippingAddressesOnCart` metod för att ange fakturerings- och leveransadresser för kundvagnen när gästutcheckning är inaktiverad.
...
* Åtgärdade problem med `giftRegistryUid` attributet för `updateGiftRegistryItems` mutation.
...
* En ogiltig kombination av tabbar och blanksteg i dialogrutan har korrigerats `phpstan.neon` -fil. [GitHub-31239](https://github.com/magento/magento2/issues/31239)
...
* Programmet visar nu fler informativa fel när fel inträffar vid körning `bin/magento` kommandon i produktionsläge. Tidigare visade programmet inga felmeddelanden eller meddelanden som saknade information. [GitHub-32786](https://github.com/magento/magento2/issues/32786)
...
* Prestanda för `catalog_product_alert` `cron` när du kör stora tabeller (flera miljoner rader) har förbättrats. Tidigare `catalog_product_alert` har läst in alla produktvarningar, vilket orsakade ett undantagsfel som inte fanns i minnet.
...
* Navigeringspilarna (**Nästa** och **Föregående**) visas nu som väntat på butiksproduktvideor.
...
* Senaste granskningsdatum för administratör **Rapporter** > **Recensioner** > **Per produkt** visar nu korrekt granskningsdatum. Tidigare visades det datum då produkten skapades i stället för granskningsdatumet.
...
* Elasticsearch genererar inte längre något fel när sidparametern för kategori-URL överskrider sidnumreringen. [GitHub-23843](https://github.com/magento/magento2/issues/23843)
...
* Programmet avmarkerar inte längre **Lägg till kommentarer** kryssruta när en kund klickar **Få leveransmetoder och fraktpriser** och väljer en leveransmetod när du skapar en beställning från administratören. [GitHub-30256](https://github.com/magento/magento2/issues/30256)
...
* Oöversättningsbara fraser i administratören kan nu översättas. (Denna pull-begäran bidrar till pågående insatser för att göra alla Admin-strängar översättningsbara.) [GitHub-11175](https://github.com/magento/magento2/issues/11175)
...
* Justeringsproblem med flikarna på sidan Administratörsinbjudan (Administratör) **Marknadsföring** > **Privat försäljning** > **Inbjudningar**) har lösts. Tidigare var flikvisningen för inbjudningar som accepterats av en kund feljusterad. [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)
...
* Nu uppdateras `url_path` av kategorin som är tilldelad alla butiksomfång när du flyttar en kategori i kategorihierarkin. Om du flyttar en kategori i hierarkin tidigare uppstod ett fel `url_path`.
...
* Nu kan du spara en ny användarroll (Admin **System** > **Behörigheter** > **Användarroller**) med hela `Catalog` valt träd, exklusive `Edit Product Design` ( **Katalog** > **Lager** > **Produkter** > **Redigera produktdesign**). Tidigare sparade programmet inte produktändringar som gjorts av en användare i den här rollen och det här felet visades: `Not allowed to edit the product's design attributes`. [GitHub-31973](https://github.com/magento/magento2/issues/31973)
...
* CMS-hierarkinodens länkwidget i TinyMCE återger nu hierarkilänkarna som förväntat när **Ankartext** fältet är tomt.
...
**Problem**: _Fel i skyddsprofil för innehåll_. I storefront visas följande fel i konsolloggen: `The Content-Security-Policy directive frame-ancestors does not support the source expression unsafe-inline`. Prestanda för Storefront påverkas inte. <!-- MC-42613-->
