...
* **Hastighetsbegränsning är nu inbyggd i API:er** för att förhindra DoS-attacker (Denial-of-Service). Webb-API:er har nu begränsningar för storleken eller antalet resurser (standardmaxvärdet är inställt på 20 och kan konfigureras till ett annat värde baserat på affärsbehov) som kan begäras av en klient. Se [Hastighetsbegränsning](https://developer.adobe.com/commerce/webapi/get-started/api-security/) för information om hur du konfigurerar dessa begränsningar. <!-- MC-35358-->

* **ReCAPTCHA-täckningen har utökats** till att omfatta:

   * Webb-API:er som har motsvarande HTML-sidor täcks via ReCAPTCHA. (Detta utesluter webb-API:er som nås av integreringar.) ReCAPTCHA-täckning skyddar slutpunkter från skräppostattacker. När webb-API:er används av en integreringstjänst från tredje part som använder OAuth inaktiveras ReCAPTCHA.<!-- MC-34472-->
...
**PayPal Pay Later stöds nu** i distributioner som inkluderar PayPal. Den här funktionen gör det möjligt för kunder att betala för en beställning i delbetalningar varannan vecka istället för att betala hela beloppet vid köptillfället. <!-- MC-40556-->
...
* **Önsklistorna**. Mutationen [&#128279;](https://developer.adobe.com/commerce/webapi/graphql/schema/wishlist/mutations/add-items-to-cart/) addWishlistItemsToCart flyttar artiklar från den angivna önskelistan till kundens kundvagn.<!-- magento/partners-magento2ee#449-->

* **Register över** gåvor. Uppgifter som ingår inkluderar: <!-- PWA-1290-->
...
Version 2.4.3 innehåller B2B v1.3.2. Den här versionen innehåller flera felkorrigeringar. Se [Versionsinformation för B2B](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html?lang=sv-SE).
...
* Frågan `product` skriver inte längre över standardvärden för alla butiksvyer i en distribution med flera butiker när ett produktnamn uppdateras endast för en butiksvy. [GitHub-31083](https://github.com/magento/magento2/issues/31083)
...
* Konfigurerbar produktlagerstatus uppdateras nu automatiskt som förväntat när underordnad produkts lagerstatus uppdateras genom import. Tidigare uppdaterades inte produktens lagerstatus automatiskt när underordnade produkters lagerstatus uppdaterades genom import.
...
`AdminMassOrdersHoldOnPendingAndProcessingTest`(ersättning för föråldrad)`AdminMassOrdersHoldOnPendingAndProcessingTest`<!-- ENGCOM-8669-->
...
* Nivåpriset tillämpas nu på en produkt som förväntat när kvantitetsökningar är aktiverade och decimallagret är mindre än 1. Tidigare var kvantiteten för lägsta nivåpris inställd på 1.
...
* Du kan nu välja flera order att returnera från administratören. Tidigare uppstod ett fel när du klickade på Skicka **efter att** ha valt flera returer och valt Stäng i rullgardinsmenyn Åtgärder.

<!-- MC-41759-->

* Programmet skickar nu e-postavisering till kunder som förväntat när du har ändrat status för en retur med hjälp av REST-API:et.

<!-- MC-39981  magento/partners-magento2ee#442 -->

* Programmet filtrerar nu poster enligt filtervärden i rutnätet för kundreturer för administratörer. Tidigare omdirigerade applikationen dig till sidan Admin alla kunder, och genom att klicka på **knappen Återställ filter** omdirigerades du också till den här sidan. [GitHub-436](https://github.com/magento/partners-magento2ee/issues/436)
...
* Länken **Bli först med att recensera den här produkten** ändras nu som förväntat till ett antal recensioner när en administratör har godkänt en recension.

<!-- ENGCOM-8501 MC-30270-->

* Administratörer kan nu sortera produktrecensioner i avsnittet Produktrecensioner på produktredigeringssidan som förväntat. [GitHub-30270](https://github.com/magento/magento2/issues/30270)
...
* Kunder kan nu hitta en order på sidan Beställningar och returer när efternamnet slutar med ett tomt utrymme.

<!-- MC-41438-->

* Fakturor skapas nu med rätt totalsumma när en kundvagnsprisregel som tilldelar 100 % rabatt tillämpas på en order som också omfattas av katalog-, produkt- och rabattskatter och som kvalificerar sig för fri frakt. Tidigare hade ordern rätt pris, men fakturan gjorde det inte. [GitHub-30853](https://github.com/magento/magento2/issues/30853)
...
* Programmet beräknar nu partiella kreditnotaskattesummor korrekt för kreditnotor som baseras på antingen en order eller en faktura i butiker som använder PayPal Payment Pro som en betalningsgateway. Tidigare tillämpades på beställningar med flera fakturor på hela beställningens moms för annullering av partiella fakturor i kreditnotan.

<!-- MC-39521 ENGCOM-8528-->

* Applikationen skapar inte längre slumpmässiga databasdödlägen när du skickar e-postmeddelanden om nya beställningar till kunder. Tidigare uppstod dödlägen eftersom programmet sparade hela objektet och dess relaterade objekt i stället för uppdaterade SQL-kolumner. Programmet visade den här typen av fel: `SQLSTATE[40001]: Serialization failure: 1213 Deadlock found when trying to get lock; try restarting transaction`. [GitHub-31090](https://github.com/magento/magento2/issues/31090)
...
* Programmet skapar inte längre dubblettadressposter för ett kundkonto när en ny order skapas från administratören för en befintlig kund. Kryssrutan **Spara i Adressbok** har bytt namn till **Lägg till i Adressbok** och är nu avmarkerad i Admin som standard.
...
* Om du söker efter en produkt baserat på dess fullständiga eller partiella SKU i Avancerad sökning returneras nu den förväntade produkten.

<!-- MC-31395-->

* Navigeringsfilter i lager visar nu korrekta produktantal. Tidigare filtrerades inte produktantalsvärden från Elasticsearch efter katalogbehörigheter.

<!-- MC-23881 ENGCOM-8618-->

* Elasticsearch genererar inte längre ett fel när kategorins URL-sidparameter överskrider sidnumreringen. [GitHub-23843](https://github.com/magento/magento2/issues/23843)

<!-- MC-24033  magento/magento2#3274-->

* Nu kan du lägga till en anpassad Elasticsearch-fältmappare i `Magento\Elasticsearch\Model\Adapter\FieldMapper\Product\FieldProvider\FieldName\Resolver\CompositeResolver`. [GitHub-24350](https://github.com/magento/magento2/issues/24350)
...
* Programmet genererar inte längre ett fel när en handlare försöker skicka en order med DHL när **kryssrutan Skapa fraktsedel** är aktiverad och produktnamnet innehåller unicode-tecken. Tidigare visade programmet det här felet när du begärde att en etikett skulle skapas: `The response is in wrong format`. [GitHub-31032](https://github.com/magento/magento2/issues/31032)

<!-- ENGCOM-8952 -->

* Du kan göra en beställning från administratören i en distribution på flera platser där `United States` den är aktiverad på en webbplats och`Disable all countries` är aktiverad som standardomfång på den andra webbplatsen. Tidigare gjorde programmet inte beställningen och visade det här felet: `Please check the shipping address information. "regionId" is required. Enter and try again`. [GitHub-30577](https://github.com/magento/magento2/issues/30577)
...
* Programmet visar nu det här meddelandet när du försöker välja `Website` som standard när `Store View` det är inaktiverat när webbplatsen skapas: `Please enable your Store View before using this Web Site as Default`. Tidigare kraschade webbplatsen och programmet visade ingen varning.

### Skatt

<!-- MC-39571-->

* Applikationen tar nu hänsyn till dold skatt under valideringen av det lägsta beställningsbeloppet.
...
* Applikationen visar nu ett **fält för momsnummer** på kundregistreringssidan när `customer/create_account/vat_frontend_visibility` den är aktiverad. [GitHub-31326](https://github.com/magento/magento2/issues/31326)

### Test

<!-- MC-41393-->

* Testmiljöerna har uppgraderats till Redis 6.0.12.

<!-- ENGCOM-8529-->

* Tas bort `CacheCleaner::cleanAll();` från integreringstester. [GitHub-31074](https://github.com/magento/magento2/issues/31074)
...
* Kundinloggningssidan visar inte längre det här meddelandet när inställningen Flytta JS-kod **längst ned på sidan** (**Store** > **Configurations** > **Advance** > **Developer** > **JavaScript-inställningar**) och cookies är aktiverade: `The store will not work correctly in the case when cookies are disabled`.
...
* Nu visas sidelementen på samma sätt på butikssidor som använder Adobe Commerce standardteman. Tidigare användes inte alla format i ett tomt tema, så alla sidelement visades inte på sidor med det här temat. (Det fanns till exempel ingen förstoringsikon i sökfältet på sidan Mina beställningar.)

### Översättning och språk

<!-- MC-23553 ENGCOM-8839-->

* Hakparenteser som läggs till i strängar är inte längre undantagna när infogad översättning har konfigurerats. Tidigare konverterade `escapeHtmlAttr` metoden parenteserna till HTML-entitetskoder. [GitHub-32000](https://github.com/magento/magento2/issues/32000)

<!-- ENGCOM-8694-->

* Oöversättbara fraser i Admin kan nu översättas. (Den här pull-begäran bidrar till det pågående arbetet med att göra alla administratörssträngar lokaliserbara.) [GitHub-11175](https://github.com/magento/magento2/issues/11175)

<!-- ENGCOM-8584-->

* Textsträngar i mallen som används för att hantera lagrade betalningsmetoder (**Mitt konto**  >  **Lagrade betalningsmetoder** ) kan nu översättas. [GitHub-31211](https://github.com/magento/magento2/issues/31211)

<!-- MC-30798 ENGCOM-8717-->

* E-postmeddelanden för beställningar som skickas från administratören använder nu butikens språk, inte de språk som är kopplade till administratörens konto. [GitHub-26521](https://github.com/magento/magento2/issues/26521)

### UI

<!-- MC-41784-->

* Admin-sidfoten visar nu rätt produktversion.
...
* Nyp-för-att-zooma-gester fungerar nu som förväntat i förstoringsglaset för produktsidans bildgalleri på iOS-enheter.
...
* Anomalier med visning av kundvagnen när zoomning har åtgärdats. Tidigare överlappade visningselementen varandra när den här sidan zoomades.

<!-- MC-38235 magento/partners-magento2ee#492 -->

* Justeringsproblem med flikarna på sidan Administratörsinbjudan (Admin **Marketing** > **Private Sales** > **Invitations**) har lösts. Tidigare var flikvisningen för inbjudningar som accepterats av en kund feljusterad. [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)
...
* Programmet använder nu rätt e-postmall när e-post skickas till nya användare. Tidigare använde applikationen standardmallen även när en anpassad mall valdes.
...
**Problem**: _JavaScript-fel när reCAPTCHA är inaktiverat_. Om reCAPTCHA är inaktiverat för utcheckning fortsätter utcheckningen, men programmet visar ett `Uncaught TypeError`-fel i konsolloggen.  Det här problemet kommer att åtgärdas i en senare version. <!-- MC-42589 42649-->
...
**Problem**: Knappen Lägg **till i beställning** fungerar inte för produkter som läggs till i varukorgen av SKU.__  applikationen visar detta felmeddelande när du klickar på **knappen Lägg till i beställning** efter att ha lagt till produkter i beställningen efter SKU: `An error has happened during application run`. Produkter läggs inte till i varukorgen. **Lösning**: Använd funktionen Lägg till produkter. <!-- AC-345-->

**Problem**: GraphQL `category` och `categoryList` frågor ignorerar kategoribehörigheter som avgör om kategorier i en delad katalog är dolda eller exponerade. Skyltfönstret visar för närvarande alla kategorier som har tilldelats eller inte tilldelats den delade katalogen. Detta påverkar distributioner av Adobe Commerce 2.4.3 som implementerar en PWA storefront för vilken delade B2B-kataloger har aktiverats. <!-- MC-42528-->
