...
Adobe Commerce 2.4.8 ger förbättrad säkerhet, kompatibilitet med PHP 8.4 och MariaDB 11.4, omfattande förbättringar av GraphQL API för att snabba upp migreringen till en ny [Adobe Commerce Storefront](https://experienceleague.adobe.com/developer/commerce/storefront/) med Edge Delivery och över 500 kvalitetskorrigeringar och förbättringar som ökar funktionaliteten. Uppgradera till 2.4.8 så att din plattform är säker, anpassad efter de senaste tekniska framstegen och stöds fram till april 2028.

>[!BEGINSHADEBOX]

**Rekommendationer för godkännande**

När du planerar en uppgradering till Adobe Commerce 2.4.8 bör du tänka på dina affärsprioriteringar, din nuvarande version och dina planer på att införa nya [Adobe Commerce as a Cloud Service](https://experienceleague.adobe.com/en/docs/commerce/cloud-service/overview).
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>I enlighet med vår <a href="/help/release/lifecycle-policy.md">supportpolicy</a> är Adobe Commerce nu kompatibelt med följande LTS-versioner (long-term support) av följande databastekniker:
              <ul>
                ...
                <li><strong>MySQL 8.4 LTS</strong> <em>_(stöds fram till 2032)_</em>: Den tidigare versionen (MySQL 8.0) når slutet av sin livslängd 2026, vilket gör denna uppgradering nödvändig för att upprätthålla systemets integritet och prestanda. MySQL 8.0 stöds fortfarande, men Adobe rekommenderar att du uppgraderar till MySQL 8.4 när du uppgraderar till Adobe Commerce 2.4.8</li>
              </ul>
        ...
        <tr>
            ...
            <td>Den här versionen har stöd för RabbitMQ 4.x. Innan du uppgraderar till RabbitMQ 4 måste Adobe Commerce Cloud och lokala kunder <a href="https://www.rabbitmq.com/docs/3.13/migrate-mcq-to-qq">migrera från [!DNL classic mirrored queues] till [!DNL quorum queues]</a>. Den här migreringen krävs eftersom:
              <ul>
                ...
                <li>[!DNL Quorum queues] Upprätthåll hög tillgänglighet genom att replikera köinnehåll över flera noder</li>
              </ul>
            </td>
        </tr>
        <tr>
            <td><strong>PHP (PHP)</strong></td>
        ...
        <tr>
            ...
            <td>Adobe Commerce är nu optimerat för OpenSearch 2.19 och är inte längre kompatibelt med Elasticsearch. Alla moduler och klasser i Elasticsearch 7 och 8 är nu föråldrade i kodbasen. Adobe rekommenderar starkt att man går över till OpenSearch för både lokala installationer och molninfrastrukturer för att säkerställa fortsatt support och kompatibilitet. Se <a href="/help/upgrade/prepare/opensearch-migration.md">Migrera till OpenSearch</a>.
              <ul>
                <li>Alternativen Elasticsearch 7 och Elasticsearch 8 är nu märkta som "(Föråldrad)" i Admin-konfigurationen.</li>
                <li>När en användare väljer Elasticsearch som sökmotor i Admin-konfigurationen visar Commerce ett meddelande som <em>säger"Det här sökmotoralternativet stöds inte längre av Adobe. Vi rekommenderar att du använder OpenSearch som sökmotor i stället."</em></li>
              </ul>
            </td>
        </tr>
    </tbody>
...
<table style="table-layout:auto">
    <tbody>
        ...
        <tr>
            ...
            <td><!-- AC-13448 -->Systemet möjliggör nu effektivare massuppdateringar av nivåpriser utan att orsaka prestandaproblem eller att webbplatsen inte svarar när du använder REST API-slutpunkten <a kref="https://adobe-commerce.redoc.ly/2.4.7-admin/tag/productstier-prices/"><code>/V1/products/tier-prices</code></a> . Tidigare kunde uppdatering av ett stort antal priser med hjälp av den här slutpunkten resultera i prestandaproblem och att webbplatsen inte svarade.</td>
        </tr>
    </tbody>
</table>
...
<table style="table-layout:auto">
    <tbody>
        <tr>
           ...
           <td>Systemet fungerar nu utan det tidigare dolda beroendet från Catalog som introducerades av InventoryIndexer, vilket säkerställer att produktskapandet, växlingen av visningsläge, ändringen av Stock-status och andra relaterade funktioner fungerar som förväntat. Tidigare orsakade det här dolda beroendet inkonsekvenser eftersom olika entiteter synkroniserades och indexeraren använde olika entiteter.</td>
        </tr>
        <tr>
            <td><strong>Beställningar</strong></td>
            <td>För att minimera förvirring har knappetiketten [!UICONTROL Submit Comment] ändrats till [!UICONTROL Update] på <a href="https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/order-management/orders/order-processing#notes-for-this-order">orderdetaljsidan</a> .</td>
        </tr>
    </tbody>
</table>
...
<table style="table-layout:auto">
    <tbody>
        ...
        <td>
            <ul>
              ...
              <li><!--LYNX-731-->GraphQL-stöd har lagts till för att hämta kundvagnsregelsinformation. Introducerade nya butikskonfigurationer för att kontrollera exponeringen för alla och tillämpade kundvagnsregler, tillsammans med <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/cart/queries/all-rules/"><code>allCartRules</code></a>-frågan och <code>Cart.rules</code>-fältet för hämtning av aktiva kundvagnsregler.</li>
            </ul>
        </td>
       </tr>
        <tr>
           ...
           <td>Den här versionen innehåller följande förbättringar av kundhanteringen:
             <ul>
               ...
               <li><!--LYNX-773-->En ny <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/segments/"><code>customerSegments</code></a>-fråga har lagts till för att aktivera personaliseringsfunktioner.</li>
               <li><!--LYNX-729--><code>customerGroup</code>-frågan har lagts till för att returnera kundgruppsrelaterad information för gäster och kunder.</li>
               <li><!--LYNX-732-->Lagt till nya butikskonfigurationer för hantering av kundgrupper, introducerade frågan <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/all-groups/"><code>allCustomerGroups</code></a> för att hämta en lista över alla tillgängliga kundgrupper och exponerade fältet <code>Customer.group</code> för att komma åt den inloggade kundens gruppinformation.</li>
        ...
        <tr>
           ...
           <td>Den här versionen innehåller följande förbättringar av användarorderhanteringen:
             <ul>
              ...
              <li><!--LYNX-449-->Lade till en <code>OrderItemPrices</code> typ som <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-CartItemPrices"><code>CartItemPrices</code></a> <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-OrderItemInterface"><code>OrderItemInterface</code></a> liknar med flera nya fält för pris.</li>
              <li><!--LYNX-448-->Förbättrad API-funktionalitet för att slå samman gästbeställningar med kundkonton baserat på e-postmatchning. Effektiviserar orderhanteringen för återkommande kunder.</li>
              ...
              <li><!-- LYNX-524 -->Fältet har lagts till <code>customer_info</code> i typen <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-CustomerOrder"><code>CustomerOrder</code></a> . Det här fältet kräver typen <code>OrderCustomerInfo</code> , som innehåller information om kundnamnet.</li>
              ...
              <li><!--LYNX-642-->Fältet har uppdaterats <code>order.availableActions</code> för att säkerställa att <code>REORDER</code> åtgärden inte returneras om en handlare <a href="https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/shopper-tools/reorders-allow#configure-to-allow-customer-reorders">inaktiverar</a> ombeställningsfunktionen i Admin.</li>
        ...
        <td>
            <ul>
              <li><!--LYNX-730-->Butikskonfigurationer för katalogprissättningsregler har lagts till, frågan har lagts till <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/all-rules/"><code>allCatalogRules</code></a> för att hämta alla tillgängliga katalogregler och fältet har exponerats för att visa katalogregler som för närvarande tillämpas på <code>Product.rules</code> en produkt.</li>
              <li><!--LYNX-540-->Lade till ett <code>quantity</code> fält i för att ge information om <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-ProductInterface"><code>ProductInterface</code></a> lagernivå. Det visar tillgängligt lager eller null baserat på administratörsinställningarna.</li>
              <li><!--LYNX-460-->Korrigerad prisvisning för paketprodukter, vilket säkerställer korrekt pris- och valutainformation.</li>
              <li><!--LYNX-401-->Fälten <code>grouped_product_image</code> och <code>configurable_product_image</code> har lagts till i typen <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-StoreConfig"><code>StoreConfig</code></a> .</li>
            </ul>
        </td>
    </tr>
    <tr>
        <td><strong>Presentalternativ och presentkort</strong></td>
        <td>
            <ul>
              ...
              <li><!--LYNX-692-->Frågan har uppdaterats <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/orders/"><code>customerOrders</code></a> för att bearbeta <code>gift_message</code> fältet korrekt.</li>
              <li><!--LYNX-691-->Frågan har uppdaterats <a href="https://developer.adobe.com/commerce/webapi/graphql/schema/customer/queries/orders/"><code>customerOrders</code></a> så att den inkluderar fältet <code>applied_gift_cards</code> , vilket gör det möjligt att hämta tillämpade presentkortsdata som <code>applied_coupons</code>liknar .</li>
            </ul>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <ul>
                <li><!--LYNX-632-->Mutationen för indata för <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#mutation-requestReturn"><code>requestReturn</code></a> RMA-objekt har uppdaterats för att lösa inkonsekvenser mellan den returattributrelaterade frågan och mutationen.</li>
    ...
    <tr>
        <td><strong>Skatt och prissättning</strong></td>
        <td>
            <ul>
                <li><!--LYNX-633-->Typen med fält har uppdaterats så att de <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-StoreConfig"><code>StoreConfig</code></a> matchar inställningarna för skattevisning i Admin-konfigurationen.</li>
    ...
    <tr>
        ...
        <td>
            <ul>
                <li><!--LYNX-618-->Lade till en identifierare för adressboken i leverans- eller faktureringsvagnsadressen i typen <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-CartAddressInterface"><code>CartAddressInterface</code></a> .</li>
                <li><!--LYNX-617-->Fältet har lagts <code>same_as_billing</code> till i typen <a href="https://developer.adobe.com/commerce/webapi/graphql-api/index.html#definition-ShippingCartAddress"><code>ShippingCartAddress</code></a> , som anger om leverans- och faktureringsadresserna är desamma.</li>
    ...
    <tbody>
        <tr>
            ...
            <ul>
              ...
              <li><!-- BUNDLE-3403 -->För att underlätta expressbetalningen integrerar Braintree-tillägget fraktmetoder direkt i Google Pay modal så att kunderna kan slutföra utcheckningen med färre klick. Leveransalternativen visas och beräknas automatiskt baserat på den adress som sparats i kundens Google Pay-konto. Tidigare omdirigerades kunder till en recensionssida för att välja en leveransmetod innan de slutförde sin betalning. Nu är granskningssidan valfri, vilket ger handlaren flexibilitet att aktivera/inaktivera det här steget i kassaprocessen. Google Pay Express-knapparna finns på minivagnen, kundvagnen och högst upp i kassan.</li>
              <li><!-- BUNDLE-3402 -->Uppdaterade det aktuella Apple Pay-märket till den senaste versionen.</li>
              <li><!-- BUNDLE-3401 -->Det aktuella Google Pay-märket har uppdaterats till den senaste versionen.</li>
              <li><!-- BUNDLE-3400 -->Sofort- och Girobetbetalningsmetoderna har tagits bort eftersom de inte längre stöds.</li>
            </ul>
            </td>
        </tr>
        <tr>
            <td><strong>Spårning av paket</strong></td>
            <td><!-- BUNDLE-3420 -->När en PayPal-beställning skickas skickar Braintree-tillägget nu transportören och spårningsnumret till både handlarens och kundens PayPal-konton. Att tillhandahålla spårningsinformation och detaljer på artikelnivå förbättrar kundupplevelsen avsevärt samtidigt som driftskostnaderna för handlare minskar. Kunder kan enkelt logga in på sitt PayPal-konto, se beställningen och komma åt spårningslänken. Handlare drar nytta av automatisk lösning av tvister om "Artikeln har inte mottagits" PayPal, snabbare tillgång till pengar vid betalning och spärrade tvister, uppdateringar om spårning i realtid, tvister på artikelnivå (i stället för tvister för hela beställningen) och förbättrade riskprofiler för handlare.</td>
        </tr>
        <tr>
