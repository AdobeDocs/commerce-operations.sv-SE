...
* Sårbarheten som åtgärdas av `MDVA-43395_EE_2.4.3-p1_COMPOSER_v1.patch` och `MDVA-43443_EE_2.4.3-p1_COMPOSER_v1.patch` har åtgärdats i den här versionen.

* `AC-384_Fix_Incompatible_PHP_Method__2.4.3_ce.patch`. Den här korrigeringen åtgärdar ett allvarligt PHP-fel som uppstod under uppgraderingen. Se artikeln [Adobe Commerce upgrade 2.4.3, 2.3.7-p1 PHP Fatal error Hotfix](https://support.magento.com/hc/en-us/articles/4408021533069-Adobe-Commerce-upgrade-2-4-3-2-3-7-p1-PHP-Fatal-error-Hotfix) Knowledge Base.
...
* Integreringstoken kan inte längre användas för autentisering av API Bearer-token. Tidigare kunde en integreringstoken användas som en fristående nyckel för tokenbaserad autentisering. Det här beteendet har dock inaktiverats som standard på grund av säkerhetsaspekterna för en åtkomsttoken som aldrig förfaller. Det tidigare beteendet kan aktiveras via kommandoraden eller Admin. Se [Tokenbaserad autentisering](https://developer.adobe.com/commerce/webapi/get-started/authentication/gs-authentication-token/). <!-- AC-1619 -->
...
* HTTPS är nu aktiverat som standard för Adobe Commerce Store. **Använd säkra URL:er på Storefront** och **Använd säkra URL:er i Admin**-inställningarna är aktiverade som standard, och alla inbyggda cookies är nu inställda som säkra.  <!-- AC-1173-->
...
* **Fullständig GraphQL-täckning för överlåtbara offerter**.  B2B-företagsanvändare kan nu slutföra alla uppgifter som rör överlåtbara offerter med GraphQL. Tidigare versioner av det här API:et hade stöd för förhandlingsflöden men inte utcheckning. <!-- PWA-2101-->
...
* **Översättningsproblem i GraphQL med arkiv** för flera webbplatser och flera språk har åtgärdats. GraphQL-matcharen returnerar nu översatta strängar baserat på lagringsomfång som förväntat. <!-- PWA-1946-->
...
Med undantag av [Braintree](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree) har alla tillägg som paketerats av leverantörer tagits bort från kodbasen i Adobe Commerce 2.4.4. Handlare bör migrera till de officiella tilläggen som är tillgängliga på [Commerce Marketplace](https://marketplace.magento.com/extensions.html). <!-- AC-1165-->
...
Den nya `Magento_ElasticsearchCatalogPermissionsGraphQl` modulen inaktiveras under den uppdaterade Live Search-installationen[&#128279;](https://experienceleague.adobe.com/docs/commerce/live-search/onboard/install.html). Befintliga Live Search-installationer måste uppgraderas till Live Search 2.0.0 för att du ska kunna dra nytta av dessa funktioner. Mer information finns i Viktig information[&#128279;](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html) om Live Search.

### Uppdateringar om tillgänglighet
...
* Ikonbilder som förmedlar mening är nu ett textalternativ.
* Förbättrad kontrast i Admin-knappar och formulärfält till ett kontrastförhållande på minst 3:1 med intilliggande färger.

### Byggare av sidor

* Handlare kan nu lägga till alternativ text (`alt_text`) till bilder (bild, banner, bild) för att förbättra innehållets tillgänglighet. [GitHub-746](https://github.com/magento/magento2-page-builder/issues/746) <!-- PB-1193-->
...
* Textalternativ har lagts till i pennikonen som visas när en handlare redigerar inmatning för dragspelet Sökmotoroptimering.

<!-- AC-1272-->

* Förstoringsglasikonen som används för att utföra sökningar i hela produktgränssnittet har tilldelats ett tillgängligt namn och textalternativ.

<!-- AC-1286-->

* Produktsidan **Ny Visa** textinmatningsfält har nu ett tillgängligt namn.

<!-- AC-1283-->

* Det **här objektet har viktvalsindata** på **sidan Katalog** > **produktinformation** har nu synliga etiketter och ett tillgängligt namn.
...
* Gästshoppare kan nu checka ut med en kundadress som innehåller flera typer av anpassade kundadressattribut. Tidigare var värden för anpassade attribut inte korrekt formaterade när de visades i kassan. [GitHub-30290](https://github.com/magento/magento2/issues/30290)

<!-- MC-24379-->

* Adobe Commerce tillämpar nu tröskelvärdet för fri frakt korrekt på beställningar när tabellpriser är aktiverade. [GitHub-21832](https://github.com/magento/magento2/issues/21832)

<!-- AC-271-->

* Du kan nu använda `POST /V1/guest-carts/:cartId/items` för att lägga till enkla produkter med olika alternativ i en varukorg. Tidigare ändrades antalet för den första enkla produkten du lade till i stället för att en andra enkel produkt med de angivna alternativen lades till. [GitHub-32302](https://github.com/magento/magento2/issues/32302)

<!-- AC-1089-->

* Avsnittet Ordersammanfattning på kassasidan visar nu rätt valuta och belopp när en distribution har konfigurerats för `Poland` land och `PLN` valuta. Tidigare visade kundvagnen och kassasidan beloppet **PLN 0**.

<!-- MC-32805-->

* Adobe Commerce genererar inte längre ett fel när en kund klickar på **Uppdatera kundvagn** efter att ha ändrat en produktkvantitet i minikundvagnen. Tidigare visade Adobe Commerce det här felet:  `The quote item isn't found. Verify the item and try again`.

<!-- MC-43176-->

* GraphQL-frågor `products` för konfigurerbara produkter i en specifik butik returnerar nu endast prisintervallen för produkter i den angivna butiken. Tidigare returnerade begäranden prisintervall för enkla produkter i icke-angivna butiker.
...
* Listrutan för fältet **Tillstånd** på sidan Skapa ordning för administratör fylls nu i med de fördefinierade värdena för det valda landet. Tidigare var det här fältet ett inmatningsfält i stället för en listruta.
...
* Produktlistor återges nu korrekt i Admin i distributioner med produkter tilldelade till både en överordnad kategori och dess underkategorier, men i olika positioner inom varje kategori. Tidigare misslyckades återgivningen av produktlistan med det här felet: `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`. [GitHub-33145](https://github.com/magento/magento2/issues/33145)
...
* Produktlistor inkluderar nu korrekt underordnade produkter till en produkt som är slut i lager när den globala flaggan för att visa produkter som är slut i lager är aktiverad. Tidigare inkluderade produktlistan inte den underordnade produkten om ett av flera underordnade produktalternativ var slut i lager när den här flaggan aktiverades.

<!-- MC-42340-->

* Du kan nu använda `addConfigurableProductsToCart` mutationen för att lägga till en konfigurerbar produkt i kundvagnen i en butiksvy som inte är standard. Tidigare kunde du inte lägga till en konfigurerbar produkt i en varukorg när webbplats-ID och butiks-ID skilde sig åt. [GitHub-30948](https://github.com/magento/magento2/issues/30948)

<!-- MC-43051-->

* GraphQL-frågor `products` returnerar inte längre data om inaktiverade underordnade produkter för konfigurerbara produkter.

<!-- ACP2E-101-->

* Att skapa nya konfigurationer för en konfigurerbar produkt åsidosätter inte längre ändringar i befintliga konfigurationer. Tidigare åsidosattes statusen för befintliga varianter med standardstatusen `enabled` efter att nya varianter hade genererats.
...
* Adobe Commerce tar nu hänsyn till webbplatsens omfattning för administratörsspråk när order skapas. Tidigare fungerade beställningsinformation som kundadressattribut korrekt på en webbplats endast i en multisitedistribution. [GitHub-23254](https://github.com/magento/magento2/issues/23254)

### Kundsegment

<!-- MC-42241-->

* Antalet matchade kunder för ett nytt kundsegment uppdateras nu automatiskt när du skapar ett företagskonto. Tidigare var du tvungen att uppdatera segmentdata för att få ett korrekt antal.

### E-post

<!-- MC-42592-->

* Valideringsfel för e-postfält i popup-fönstret för inloggning i kassan matchar nu e-postvalideringsfel på kundens inloggningssida när en kund försöker logga in med ett ogiltigt e-postkonto.

<!-- MC-42764-->

* Adobe Commerce innehåller inte längre ett felmeddelande i e-postmeddelandet med sändningsinformation när en leveranskommentar läggs till från Admin och **kryssrutan Meddela kund** är aktiverad. Tidigare innehöll e-postmeddelanden med leveransbekräftelse detta meddelande: `We're sorry, an error has occurred while generating this content`.

<!-- MC-42486-->

* Region- och lands-ID-värden konverteras nu korrekt i e-postmallar. Tidigare visades inte land och regionnamn korrekt när du klickade på länken **Förhandsgranska** i e-postmallen för administratörer.
...
* Asynkrona ordermeddelanden som skickas från en icke-standardwebbplats innehåller nu logotyp-URL:er från lämplig webbplats. Tidigare innehöll dessa e-postmeddelanden alltid logotyper från standardwebbplatsen eller den primära webbplatsen. (En standardnyckel med ett `storeCode` värde har tilldelats det andra matrisargumentet som skickas till `UrlInterface` `getUrl` funktionen. Det här värdet särskiljer URL:er med avseende på olika butiker/webbplatser och genererar en korrekt `baseUrl` för det e-postinnehåll som skickas till kunden.)
...
* Adobe Commerce kopierar nu alla produktfält enligt definierade omfattningar när en produkt dupliceras. Tidigare återställdes fälten name **och** description **till `global` omfånget**.

<!-- MC-42612-->

* Adobe Commerce använder inte längre deltaavrundning för en rabatt när produktpriset är 0. Tidigare beräknade Adobe Commerce ett negativt rabattbelopp.

<!-- MC-42215-->

* Produkter med attributet **Ange som ny** som har tilldelats ett tomt startdatum och ett utgånget slutdatum kan nu sparas. Tidigare utlöste Adobe Commerce det här felet när du sparade en produkt med följande inställningar: `Make sure the To Date is later than or the same as the From Date`.
...
* Administratörer kan nu försöka utföra åtgärder som har körts över den maximala bearbetningstiden. Standardvärdet är 12 timmar.

<!-- MC-42514-->

* Brutna pipe-fel uppstår inte längre under bearbetning av massåtgärder på grund av obekräftade meddelanden för konsumenten. En prefetch count-egenskap begränsar nu dessa meddelanden för konsumenten och undviker fel. Tidigare genererades det här felet när du körde `bin/magento queue:consumers:start async.operations.all` :  `Broken pipe or closed connection`.

<!-- MC-42075-->

* Shoppare omdirigeras nu tillbaka till inloggningssidan som förväntat efter ett andra misslyckat inloggningsförsök. Tidigare omdirigerades shoppare till en 404-sida efter ett andra misslyckat inloggningsförsök. [GitHub-32885](https://github.com/magento/magento2/issues/32885)
...
* Åtgärdat regressionsproblem som påverkade meddelanden som lämnades i undantagsloggarna för formuläret Kontakta oss. [GitHub-34483](https://github.com/magento/magento2/issues/34483)
...
* Klassen `BundleDiscountPrice` i `magento2/app/code/Magento/Bundle/Pricing/Price/ConfiguredPrice.php` deklareras nu som förväntat. Tidigare utlöste Adobe Commerce det här felet: `Class Magento\Bundle\Pricing\Price\BundleDiscountPrice not found`. [GitHub-3334](https://github.com/magento/magento2/issues/33334)

<!-- AC-1239-->

* Undantagna CSV-fältvärden som skrivits av AWS S3-adaptern matchar nu de data som skrivits av andra filsystemkort.
...
* `generateCustomerToken`- och `revokeCustomerToken`-mutationerna ger nu korrekt cache-ID för användartillståndet. Tidigare använde dessa mutationer en ogiltig `x-magento-cache-id`, vilket försämrade prestandan.

<!-- PWA-1506-->

* Mutationen `setBillingAddressToCart` `same_as_shipping` använder nu parametern korrekt för att ställa in faktureringsadressen så att den matchar leveransadressen som förväntat. Tidigare visade Adobe Commerce det här felet: `The shipping method is missing. Select the shipping method and try again`. [GitHub-30924](https://github.com/magento/magento2/issues/30924)
...
* Vattenstämpelbilder med genomskinlig bakgrund har inte längre en vit bakgrund på den transparenta produktbilden som den överlägger. Tidigare, när både vattenstämpelbilden och produktbilden hade en genomskinlig bakgrund, visades vattenstämpeln med en vit bakgrund.

<!-- AC-1240-->

* Logotyper för fakturor och leveranskvitton laddas nu upp och visas som förväntat när AWS S3 är aktiverat.

<!-- MC-42160-->

* Den medietagg som är associerad med en bild som lagts till på en CMS-sida i en distribution där Admin är inställd på en annan domän än butikens URL innehåller nu butiks-URL:er som förväntat. Tidigare innehöll medietaggar statiska administratörs-URL:er i stället för förväntade butiks-URL:er. [GitHub-32930](https://github.com/magento/magento2/issues/32930)
...
* `bin/magento remote-storage:sync` misslyckas inte längre när filnamnen liknar varandra men bara skiljer sig åt med ett eller två tecken. Tidigare lagrades den första bilden korrekt, men den andra bilden med liknande namn utlöste ett synkroniseringsfel och det här felet: `Notice: Undefined index: media/catalog/product/5/_ in /src/vendor/magento/module-remote-storage/Driver/Adapter/Cache/Generic.php on line 197`. [GitHub-34337](https://github.com/magento/magento2/issues/34337)

<!-- MC-41672-->

* Adobe Commerce konverterar nu tidsstämpeln i exportfilnamnet till användarens tidszon efter en schemalagd export. Tidigare konverterades inte dessa värden och Adobe Commerce visade UTC-tidsstämpeln.

<!-- MC-42330-->

* Adobe Commerce skapar inte längre dubbletter av bilder i fjärrlagring när samma CSV-fil importeras mer än en gång.
...
* Kodbasen har omstrukturerats för att ta bort anrop till föråldrade metoder `phpunit/phpunit`för . [GitHub-33916](https://github.com/magento/magento2/issues/33916)
...
* `jquery.cookie` bibliotek från tredje part (senaste versionen) [GitHub-34427](https://github.com/magento/magento2/issues/34427) <!-- AC-101-->

#### Uppdateringar av composer-beroenden
...
* Sniffningen `Magento2.Annotation.MethodAnnotationStructure` misslyckas inte längre med ett falskt positivt resultat. [GitHub-34679](https://github.com/magento/magento2/issues/34679)

### Media Galleri

<!-- AC-1054-->

* Kommandot `bin/magento media-gallery:sync` misslyckas inte längre i distributioner där AWS S3 är aktiverat.
...
* Fakturor för ordrar som betalas med butikskredit och som har fasta produktskatter (FPT) inkluderar nu den korrekta totalsumman.

<!-- MC-42332-->

* Detaljsidan för administratörsorder läses nu in som förväntat. Tidigare utlöste Adobe Commerce följande fel vid inläsning av orderinformationssidan för ordrar med vissa skatter: `Call to a member function getId() on array`.

<!-- MC-38521-->

* Administratörer med begränsade behörigheter som inkluderar order- och leveransprivilegier kan nu visa ordersidan som förväntat. Tidigare utlöste Adobe Commerce ett fel när en administratör med dessa behörigheter försökte visa en order. [GitHub-14633](https://github.com/magento/magento2/issues/14633)

<!-- ACP2E-74-->

* Kunder kan nu lägga en order som innehåller en underordnad konfigurerbar produkt när en högsta tillåten kvantitetsgräns har angetts.
...
* PayPal Express fungerar nu som förväntat från kundvagnen. När du tidigare klickade på knappen **PayPal** för att starta en expresskopia från kundvagnen uppstod följande fel i Adobe Commerce: `To check out, please sign in with your email address`. [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
### ProductAlert (på engelska)

<!-- MC-41917-->

* Den nya `product_alert` konsumenten förbättrar sändningen av kundaviseringar genom att skapa kömeddelanden, köra konsumenten och förbättra körningstiden. Tidigare utlöste Adobe Commerce ett undantag för slut på minne när fler än 100 000 produktaviseringar skickades. Det tog också mer än 20 timmar för Adobe Commerce att skicka alla aviseringar.
...
* `category_name` har lagts till i de systemreserverade produktattributkoderna. Adobe Commerce visar nu ett fel om en användare försöker skapa ett produktattribut med attributkoden `category_name`. Tidigare listades inte produkter i den förväntade kategorin och ett Elasticsearch-fel loggades.
...
* Adobe Commerce lägger nu till rätt prisfält i Elasticsearch prisindexmappning. Tidigare lades felaktiga prisfält till i indexet med butiks-ID i stället för webbplats-ID.
...
* Antalet förfrågningar till transportföretag online har reducerats från flera förfrågningar till en. Paketets försäkrings- och hanteringspris per artikel/paket beräknas nu korrekt. Tidigare visade Adobe Commerce felaktiga fraktkostnader för FedEx-fraktmetoder.
...
* Förbrukningen av integrationstestminne har minskats, vilket förbättrar testprestanda och minskar den tid som krävs för att slutföra testet.

<!-- MC-41955-->

* Lade till ett test för att verifiera anpassat datumattributformat för skyltfönster och administratör.

<!-- AC-1517-->

* Rättade fel med `StorefrontPOWorkflowVerifyApprovalFlowTabTest`. [GitHub-34378](https://github.com/magento/magento2/issues/34378)

<!-- AC-1571-->

* Funktionella tester har stabiliserats för PHP 8.x-kompatibilitet. [GitHub-34327](https://github.com/magento/magento2/issues/34327), [GitHub-34188](https://github.com/magento/magento2/issues/34188)
...
* Testet `Magento\GraphQl\CatalogGraphQl\ProductSearchTest` genererar inte längre det här felet när det körs med AWS Elasticsearch: `Magento\GraphQl\CatalogGraphQl\ProductSearchTest::testSearchSuggestions Failed asserting that an array is not empty. /var/www/html/dev/tests/api-functional/testsuite/Magento/GraphQl/CatalogGraphQl/ProductSearchTest.php:94 /var/www/html/dev/tests/api-functional/framework/Magento/TestFramework/TestCase/GraphQlAbstract.php:257`.

<!-- magento/magento2/pull/33699-->

* Korrigerade ett stort antal icke-kritiska enhetstestvarningar genom att byta namn på alla fingerade variabler så att suffixet inkluderades och `Mock` referenser till en inaktuell Object Manager-klass togs bort. [GitHub-33695](https://github.com/magento/magento2/issues/33695)

* Testet `Magento\Framework\MessageQueue\TopologyTest` är nu kompatibelt med AWS MQ för [!DNL RabbitMQ] tjänst. Tidigare misslyckades det här testet med det här meddelandet: `Invalid exchange configuration: magento-topic-based-exchange1 Failed asserting that two arrays are equal`. <!-- AC-1248-->
...
* Du kan nu ange ett kunddatum när du skapar ett nytt konto i butiker som är inställda på portugisiska (Portugal). Tidigare utlöste Adobe Commerce ett fel när du försökte skapa ett konto. [GitHub-34130](https://github.com/magento/magento2/issues/34130)

<!-- ACP2E-457-->

* Produktinformationssidan visas nu med den arabiska (Saudiarabien) språkinställningen. Tidigare bröt den här språkinställningen produktinformationssidan.

### UI

<!-- AC-258-->

* Rättade fel med MFTF-tester som använder `maps.googleapis.com`.
...
* URL-omskrivningar genereras inte längre om för alla butiksvyer när en ny butiksvy skapas när den körs `bin/magento setup:upgrade`. [GitHub-32954](https://github.com/magento/magento2/issues/32954)

### Visuell Merchandiser

<!-- ACP2E-19-->

* Det automatiska sorteringsalternativet Visual Merchandiser **Move out of stock to bottom** fungerar nu som förväntat för konfigurerbara produkter när alla enkla produkter är slut i lager.

<!-- ACP2E-61-->

* Visual Merchandiser-regeln sparas inte längre när inga villkor har lagts till.

### Ramverk för webb-API

<!-- MC-42008-->

* Webb-API-begäranden för självauktoriserade kundresurser genererar inte längre auktoriseringsfel när beständig kundvagn är aktiverad.

<!-- MC-42313-->

* Du kan nu skapa objekt med hjälp av `\Magento\Framework\Api\AbstractSimpleObjectBuilder` de underordnade klasserna i PHP 7.3. Tidigare `preg_match` kastades detta varningsmeddelande när Adobe Commerce var värd på Redhat med PHP 7.3: `Warning: preg_match(): Compilation failed: unrecognized character follows...`.

<!-- MC-24548-->

* Summorna som hämtas `PUT /V1/guest-carts/:cartId/collect-totals` av begäran uppdateras nu med rätt `shippingMethod`. [GitHub-18508](https://github.com/magento/magento2/issues/18508)
...
* Adobe Commerce genererar nu en `customertoken` av GraphQL- eller REST API-begäranden som förväntat efter flera på varandra följande misslyckade inloggningsförsök. Tidigare kontrollerade Adobe Commerce inte om värdet `lock_expires_at` för in `oauth_token_request_log` var större än det aktuella datumet och tiden, och returnerade alltid antalet misslyckade försök, vilket hindrade kunden från att någonsin logga in. [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* PUT-anropet `/rest/all/V1/categories/<above_created_category_id>` respekterar nu de värden som anges av `available_sort_by`.  Tidigare var listrutan Sortering efter för tillgängliga produkter markerad med alla möjliga alternativ på sidan Kategori och **kryssrutan Använd alla** var markerad. [GitHub-32596](https://github.com/magento/magento2/issues/32596)

<!-- ACP2E-11-->

* Prestandan för REST API-begäranden för att skapa kundtoken har förbättrats.

<!-- ACP2E-91-->

* Åtgärdade ett fel som uppstod när **Lägg till butikskod i URL:er** är inställt på **Ja** och ett REST-anrop inte innehöll en butikskod. Trots att nyttolasten var giltig returnerade anropet följande fel: `Fatal error: Allowed memory size of **&#x200B;**&#x200B;**&#x200B;**&#x200B;**&#x200B; bytes exhausted (tried to allocate &#x200B;**&#x200B;** bytes) in vendor/magento/module-store/Model/StoreRepository.php on line 75`.
...
**Problem**: Säljare kan inte skicka in partiella återbetalningar för beställningar som betalats med Apple Pay via Braintree. När en handlare försöker skapa en kreditnota för en partiell återbetalning från orderfakturan kan fältet **Antal att återbetala** inte redigeras.  **Lösning**: Använd patch `braintree-disabled-partial-capture-for-applepay-googlepay.patch`. Se artikeln [Adobe Commerce 2.4.4: Det går inte att skapa partiella fakturor](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) i kunskapsbasen.  <!-- BUNDLE-3088-->

**Utgåva**: Köpare kan inte lägga till en produkt i kundvagnen när inga alternativ har valts i Admin **Store** > **Konfiguration** > **Allmänt** > **Lands-alternativ** > **Tillåt länder**. Adobe Commerce visar i stället följande konsolfel: `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->
...
Dotdigital är en plattform för kundengagemang som hjälper digitala marknadsförare och utvecklare att leverera kommunikation under hela kundresan. Vi utnyttjar kraften i kunddata och driver engagemang, konvertering och lojalitet för varumärken när de växer och skalas upp. Kunderna älskar vår lättanvända plattform som kopplar samman förstapartsdata mellan systemen, ger kraftfulla insikter och automatiserar prediktiva meddelanden över flera kanaler.

Dotdigitals 350+ anställda betjänar medelstora företag och företagsföretag runt om i världen och inom olika branscher. Vi strävar efter att inspirera till ansvarsfull marknadsföring och är engagerade i hållbarhet, integritet och säkerhet. Dotdigital är stolta över att vara världens första koldioxidneutrala plattform för marknadsföringsautomation, certifierad för ISO 14001, ISO 27701 och ISO 27001.
...
Community Engineering-teamet [Magento Contributors](https://magento.biterg.io/app/kibana) har en lista över de mest bidragsgivande personerna och partnerna per månad, kvartal och år. Från sidan Deltagare kan du följa länkar till deras sammanfogade PR:er på GitHub.

### Bidrag från partner

I följande tabell visas bidrag som lämnats av partner. I den här tabellen visas den partner som bidrog med pull-begäran, numret för den externa pull-begäran och det GitHub-problemnummer som är associerat med det (om det är tillgängligt).

{{$include /help/_includes/release-notes/engcomm-2-4-4-partner.md}}

### Bidrag från enskilda bidragsgivare
...
Du kan installera Adobe Commerce 2.4.4 med [Composer](../../../installation/composer.md).
