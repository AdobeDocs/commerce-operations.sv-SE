...
* Adobe Commerce 2.4.4 har stöd för Elasticsearch 7.16 och OpenSearch 1.2. Adobe Commerce handlare med driftsättningar på plats kan använda båda lösningarna. OpenSearch är nu standardsökmotorn för Adobe Commerce 2.4.4-distributioner som lagras i molnet. Alla kunder med molndistributioner som uppgraderar till version 2.4.4 måste använda OpenSearch. Se [Byt till OpenSearch för Adobe Commerce i molnet 2.4.4](https://support.magento.com/hc/en-us/articles/4419942355725-Switching-to-OpenSearch-for-Adobe-Commerce-on-Cloud-2-4-4)

* The `JQuery` har uppgraderats till version 3.6. The `jquery-ui` har uppgraderats till version 1.13.0. Flera andra JavaScript-bibliotek har uppdaterats till de senaste versionerna.

* TinyMCE 5.8.1 stöds nu. TinyMCE 4 har tagits bort från kodbasen. <!-- AC-41-->

* The `RequireJS` har uppgraderats till den senaste versionen (v2.3.6). [GitHub-33672](https://github.com/magento/magento2/issues/33672) <!-- AC-40 422-->
...
* `.zIndex()`. jQuery UI v1.12.1 innehåller `jquery/z-index.js`, som stöder `.zIndex()`. Se [UI-kärna](https://jqueryui.com/changelog/1.12.0/#ui-core).

* Datareservdelar för widgetnamn. Du måste använda det fullständiga namnet för `.data()` -tangenten.
...
* Den nya `multiple_processes` konfigurationsalternativet har stöd för parallella konsumenter i flera processer. Tidigare `cron` en enda kund vid behov. Att få flera konsumenter att köra processer parallellt kan ge snabbare körning av uppgifter. Om du vill aktivera funktionen lägger du till `multiple_processes` till `app/etc/env.php` -fil.

* Kundvagnshanteringen för varukorgar som innehåller mer än 750 konfigurerbara produkter har förbättrats genom att minnesgränsen som anges i `max_input_vars` i `php.ini` -fil som stöder volym för indatavariabler.

* Optimering av bearbetning av försäljningsregler under utcheckning genom att skjuta upp den totala beräkningen. Vanligtvis sker säljregelbearbetning varje gång en kund lägger till en produkt i kundvagnen, uppdaterar produktkvantiteten eller klickar på  **Montera beställning** -knappen. Handlare kan aktivera detta uppskjutande genom att ställa in `checkout/deferred_total_calculating` i `env.php` -fil. Du kan också köra `bin/magento setup:config:set --deferred-total-calculating 1|0`.  <!-- MCP-573-->
...
* The `bin/magento app:config:import` kommandot körs nu utan fel vid uppdatering `time` värden i `config.php`. Tidigare har en ändring av produktvarningskonfigurationen genom manuell redigering (d.v.s. inte med Admin) resulterat i ett fel.
...
* `cron` rensar nu produktkategoricachen som förväntat under `indexer_update_all_views` exekvering. Tidigare visade Adobe Commerce ett felaktigt produktantal på kategorisidan efter en massuppdatering av många produkter i kategorin.
...
* `cart` Svar på frågor ger nu rätt rabatter för paketprodukter med dynamiska priser. Tidigare innehöll frågesvaren rabatter som tillämpades på varje enskild paketprodukt.
...
* Korsförsäljningsprodukter syns inte längre på kundvagnssidan när **Visa korsförsäljningsartiklar i kundvagnen** konfigurationsinställningen är inaktiverad.
...
* Kuponggenerering blockeras nu tills den relaterade kundvagnsprisregeln sparas med `auto` aktiverat alternativ. Om du försöker spara regeln utan att aktivera `auto` Adobe Commerce visar detta meddelande: `Rule is not saved with auto generate option enabled. Please save the rule and try again`.
...
* Datatypsbyte `int` har tagits bort för automatiskt ökade faktura-ID:n när de föregås av en sträng. Därför visar nu fakturor 0 för tal som är prefix med en sträng. [GitHub-34451](https://github.com/magento/magento2/issues/34451)
...
* `CURLOPT_POSTFIELDS` godkänner inte längre objekt som arrayer. Om du vill tolka ett objekt som en array utför du en explicit (`array`). Detta gäller även andra alternativ som accepterar arrayer. [GitHub-33781](https://github.com/magento/magento2/issues/33781)
...
* Ett index har lagts till i `magento_giftcardaccount.code` för att förbättra prestanda.
...
* Adobe Commerce har nu **Spara** knappar och fält för automatisk komplettering när du anger en ofullständig URL när du lägger till en video (Admin **Katalog**  > **Produkter** > **Lägg till video**). Tidigare **Spara** inaktiverades och fälten fylldes inte i.
...
* Utgångna belöningspunkter markeras nu som **Utgånget** i avsnittet Orsak i historikrutnätet för bakåtpunkter.
...
* Adobe Commerce tar nu bort HTML-taggar som förväntat från butiken **Konto** > **Mina beställningar** > **Visa order** sida. Tidigare visade Adobe Commerce HTML-taggar i kommentardelen i butikens kundorder.
...
* Flera icke-kritiska enhetstestvarningar har korrigerats genom att alla modellvariabler döptes om till att innehålla `Mock` och ta bort referenser till en inaktuell Object Manager-klass. [GitHub-33695](https://github.com/magento/magento2/issues/33695)
...
* Tillagda `aria-label` -element till butikssidans mall för att ange till vilken sida kunden ska navigera när han/hon klickar på en länk. Tidigare användes samma länktext för länkar till olika sidor. [GitHub-33075](https://github.com/magento/magento2/issues/33075)
...
* `.action-close` fungerar nu som förväntat när `Multiselect` används i en modal. Tidigare `action-close` fungerar inte eftersom den ärver CSS för  `.action-close` spärrknappen. [GitHub-27240](https://github.com/magento/magento2/issues/27240)
...
* Validering av negativa värden har lagts till i **Antal att returnera** på formuläret Create New Return.
...
* Du kan nu ta bort en åsidosättning för en parameter som definieras i en `webapi.xml`-fil. [GitHub-33843](https://github.com/magento/magento2/issues/33843)
...
**Problem**: Köpare kan inte lägga till en produkt i kundvagnen när inga alternativ har valts i Admin **Butik** >  **Konfiguration** > **Allmänt** > **Alternativ för land** > **Tillåt länder**. Följande konsolfel visas i stället i Adobe Commerce: `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->

**Problem**: Beskrivning av Dotdigital on **Hitta partners och tillägg** > **Magento Marketplace** är föråldrad. Här är en uppdaterad beskrivning:
...
Se [Dotdigital - Automatisering av marknadsföring](https://marketplace.magento.com/dotdigital-dotdigital-magento2-os-package.html) för köp och nedladdning.
