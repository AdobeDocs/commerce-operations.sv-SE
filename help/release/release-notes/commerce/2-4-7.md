...
* **Radartikelrabatter**. Rabatterna kan nu tillämpas som en procentandel av, ett fast värde eller som utökad prissättning för specifika radartiklar inom en offert.
...
* The `addProductsToCart` frågan lägger nu till ett presentkort i en kundvagn. Tidigare förväntades presentkortets mottagarnamn, men avsändarens namn returnerades istället. Adobe Commerce orsakade detta fel: `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* `product` frågor returnerar nu bara aggregeringar för produkter som har tilldelats den angivna delade katalogen. Tidigare visades aggregeringsdata för produkter som inte har tilldelats en delad katalog i lagerstyrd navigering.
...
* Visningsproblem på butikens informationssida som berodde på katalogbehörighetsindexering har åtgärdats. Tidigare rensade Adobe Commerce indextabellen först och indexerade sedan om under en `catalogpermissions_product` eller `catalogpermissions_category` indexeringsprocess. Katalogbehörigheterna fungerade inte som väntat i butiken förrän omindexeringen är klar.
...
* Attributalternativ som tillhandahålls av datakällmodellen och installeras med hjälp av konfigurationsdatakorrigeringen (vanligtvis från tredjepartsmoduler) kan nu användas med kundsegmentsregler. Tidigare returnerade den här kombinationen inte listan över kunder som matchade det konfigurerade segmentet.
...
* Integrationstester för redigeringsfunktioner i presentregister är inte längre beroende av befintliga enhets-ID:n. Alla data som behövs hämtas nu via korrigeringar. Tidigare `\Magento\GiftRegistry\Controller\Index\EditPostTest` misslyckades.
...
* Välkomstmeddelanden som innehåller enkla citattecken återges nu korrekt på butiken utan konsolfel. Om ett välkomstmeddelande innehöll ett enkelt citattecken lästes inte välkomstmeddelandet in vid återgivning på lagerfront. Adobe Commerce visade även konsolfel.
...
* Nu kan du öppna en order från administratören med statusen `Received` när `AsyncOrder` är inte aktiverat. Tidigare gick det inte att öppna orderdetaljsidan från administratören när orderstatusen ändrades av `POST V1/orders`.
