...
Adobe Commerce 2.4.7 är fortfarande kompatibelt med PHP 8.1 endast i uppgraderingssyfte. PHP 8.1 stöds inte och rekommenderas inte. Adobe Commerce 2.4.7 Core-kod, alla programpaket och alla tillägg som ägs av Adobe samt SaaS-tjänster är kompatibla med PHP 8.3. <!-- AC-9668 -->
...
* **Flera kuponger per order**. Merchants kan nu [konfigurera maximalt antal kuponger](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/sales#multicoupon-settings) som kan tillämpas per order med den nya **[!UICONTROL Maximum number of coupons per order]** konfigurationsalternativ. Det här värdet är som standard 1. Nu kan du använda [REST](https://developer.adobe.com/commerce/webapi/rest/modules/multicoupon/) eller [GraphQL](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/apply-coupons/) för att tillämpa flera kuponger på en kundvagn.
...
Commerce 2.4.7 innehåller följande förbättringar av handelsprestanda och skalbarhet:
...
* **Snabbare besparingar av konfigurationer på butiksnivå för driftsättningar i många butiker**. Det kan ta lång tid att spara konfigurationsinställningar i distributioner med fler än 500 butiker. Den nya [Async Config](../../../performance/configuration.md#asynchronous-configuration-save) i kan du spara asynkrona konfigurationer genom att köra ett cron-jobb där en konsument bearbetar sparandet i en meddelandekö. AsyncConfig är inaktiverat som standard.
...
[GraphQL Application Server](../../../performance/application-server.md) gör det möjligt för Adobe Commerce att upprätthålla status bland Commerce GraphQL API-begäranden och eliminerar behovet av bootstrap, vilket ger högre genomströmning, lägre latens och effektiv resursanvändning för alla GraphQL API:er. Genom att dela programstatus mellan processer blir GraphQL API-begäranden betydligt effektivare, och GraphQL API-svar är i genomsnitt 30 % snabbare.<!-- ACPT-27 -->
...
Den här versionen innehåller metapaketet Adobe Commerce Extension v2.0.0, som automatiskt paketerar utvalda Adobe Commerce-tillägg med den här huvudversionen. De versioner av dessa tillägg som ingår i det här metapaketet installeras när `composer update` körs, vilket förenklar processen att uppgradera dessa tillägg när du uppgraderar till den senaste kärnversionen. Dessa tillägg har oberoende releasescheman.
...
Commerce 2.4.7 innehåller förbättrade cachningsfunktioner för GraphQL, stöd för GraphQL schema för anpassade attribut, stöd för headless-orderannullering och förbättrad cachning för lösare.
...
* **Förbättrade funktioner för GraphQL-cachning förbättrar sidinläsningshastigheten**. Cachelagring har lagts till i de här frågorna, vilket förbättrar sidinläsningstiden för de flesta PWA-sidor:
   ...
   * [`countries`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/countries/) och [land](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/country/)
...
* Arbetsflödet för utcheckning innehåller nu en avdelning för expressbetalning som innehåller PayPal, Google Pay och Apple Pay **[!UICONTROL Express]** knappar. <!-- BUNDLE-3322 -->
...
>[!NOTE]
>
>För första gången innehåller flera REST-slutpunktsflöden strängen `V2` i stället för `V1`. Till exempel: `GET /rest/default/V2/carts/{cartId}/coupons`. För handlare är dessa slutpunkter avsedda att ersätta `V1` versioner av slutpunkterna. De här slutpunkterna är bara tillgängliga i Adobe Commerce.<!-- LYNX-208 -->
...
Vi har åtgärdat hundratals problem i Adobe Commerce 2.4.7 Core-koden. En deluppsättning av de åtgärdade problemen som ingår i den här versionen beskrivs nedan. Korrigeringar som ingår i tidigare versioner av Commerce 2.4.7-beta beskrivs också.
...
* The `bin/magento varnish:vcl:generate` kommandot innehåller nu `input-file argument`. Detta stöder tillägg av en anpassad VCL i en Git-databas i förhållande till Commerce-roten. [GitHub-37363](https://github.com/magento/magento2/issues/37363)
...
* Page Builder-sidelementen återges nu med en acceptabel hastighet i webbläsaren Chrome (v112). Tidigare gick det långsamt att spara innehåll med Page Builder i webbläsaren Chrome och det här felet loggades i webbläsarkonsolen: `[ERROR] Page Builder was rendering for 5 seconds without releasing locks`.
...
Du kan installera Adobe Commerce 2.4.7 med [Disposition](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
