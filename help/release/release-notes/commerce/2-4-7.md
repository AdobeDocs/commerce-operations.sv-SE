...
* **Stöd för underresursintegritet (SRI)** har lagts till för att uppfylla PCI 4.0-kraven för verifiering av skriptintegritet på betalningssidor. Stöd för SRI (Subresource Integrity) ger integritetshash för alla JavaScript-resurser i det lokala filsystemet. Standard-SRI-funktionen implementeras endast på betalningssidorna för Admin- och skyltfönsterområdena. Handlare kan dock utöka standardkonfigurationen till andra sidor. Se [Integritet](https://developer.adobe.com/commerce/php/development/security/subresource-integrity/) för delresurser i utvecklarhandboken _för_ PHP för handel.<!--AC-1153-->

* **Ändringar i CSP (Content Security Policy)** - Konfigurationsuppdateringar och förbättringar av Adobe Commerce Content Security Policies (CSP:er) för att uppfylla PCI 4.0-kraven. Mer information finns [i Säkerhetspolicyer](https://developer.adobe.com/commerce/php/development/security/content-security-policies/) för _innehåll i Commerce PHP Developer Guide_. <!--AC-11513-->

   * Standardkonfigurationen för CSP för betalningssidor för Commerce-administratör och skyltfönster är nu `restrict` läge. Standardkonfigurationen är `report-only` för alla andra sidor.  I versioner före 2.4.7 konfigurerades CSP i `report-only`-läge för alla sidor.
...
[GraphQL Application Server](../../../performance/application-server.md) gör det möjligt för Adobe Commerce att upprätthålla tillståndet mellan Commerce GraphQL API-begäranden och eliminerar behovet av bootstrapping, vilket resulterar i högre genomströmning, lägre latens och effektiv resursanvändning för alla GraphQL API:er. Genom att dela programtillstånd mellan processer blir GraphQL API-begäranden betydligt effektivare, och GraphQL API-svar är i genomsnitt 30 % snabbare.<!-- ACPT-27 -->
...
>[!NOTE]
>
>GraphQL Application Server är inte kompatibel med [[!DNL Amazon Simple Storage Service (AWS S3)]](https://aws.amazon.com/s3/). Adobe Commerce-kunder i molninfrastruktur som för närvarande använder [!DNL AWS S3] för [fjärrlagring](../../../configuration/remote-storage/cloud-support.md) kan inte använda GraphQL Application Server förrän Adobe släpper en snabbkorrigering senare under 2024.<!-- ACPT-1840 -->

### Adobe Commerce Extension-metapaket

I den här versionen introduceras metapaketet Adobe Commerce Extension v2.0.0, som automatiskt paketerar utvalda Adobe Commerce-tillägg med den här huvudversionen. De versioner av dessa tillägg som ingår i det här metapaketet installeras när `composer update` körs, vilket förenklar uppgraderingen av dessa tillägg när du uppgraderar till den senaste kärnversionen. Dessa tillägg upprätthåller oberoende utgivningsscheman.
...
* **Utökat stöd för anpassade attribut**. Stöd för anpassade GraphQL-attribut har förbättrats genom att berika API-data för att stödja alla attributtyper. Schemat för GraphQL EAV-attribut har nu stöd för att utöka kundattribut och kundadressobjekt i Admin och hämta dem med hjälp av GraphQL. Specifika förbättringsområden är: <!-- LYNX-61 110-->
...
* Avsnittet Rapport i rutnätet Produkter i kundvagnar visar nu rätt produktpris när **[!UICONTROL Catalog Price Scope]** det är inställt på **[!UICONTROL Website]**. Tidigare visades produktpriset som noll när produkten tilldelades en annan webbplats än standardwebbplatsen.
...
* Omstrukturerat hur banderollinnehållet läses in för att ta bort överdrivna databasfrågor, vilket resulterar i förbättrade laddningstider för kundvagnen.

### Bunta ihop produkter

<!-- ACP2E-1464 -->

* Kategorins produktantal tar nu korrekt hänsyn till paketprodukter när **[!UICONTROL Display Out Of Stock*]** det är aktiverat.
...
* Commerce optimerar nu prestanda genom att inte läsa in användarspecifika offertdata när du genererar helsidessidor som kan cachelagras, t.ex. produktinformationssidor (PDP) och CMS-sidor när beständig kundvagn är aktiverad. Tidigare lästes användarspecifika sessioner in i onödan och data citerades när dessa sidor genererades, vilket påverkade prestandan.
...
* Inläsningstiderna för kassasidor har förbättrats för kunder med stora adressböcker. Commerce bearbetar nu endast det antal kundadresser som anges i inställningen **[!UICONTROL Customer Addresses Limit]** . Tidigare läste Commerce in hela adressboken.

<!-- ACP2E-2174 -->

* Produktkvantitet kan nu ändras i varukorgen som förväntat för varor som finns i lager när en artikel är slut i lager. Tidigare kunde kunder inte ändra produktkvantiteten för några varor när en var slut i lager.

<!-- ACP2E-2269 -->

* Filtret `AdvancedSalesRule` för produktattribut fungerar nu korrekt med serverdelstypen för decimalattributet. Därför fungerar kundvagnsreglerna med `Coupon Type` inställd på **[!UICONTROL No Coupon]** nu som förväntat.
...
* Gäst-API:et begär att `POST V1/guest-carts/<cartId>/shipping-information` inte längre returnera en `cancel` status om sidan laddas om under inloggning i en mobil miljö. Tidigare utlöste Adobe Commerce ett 500-fel och loggade det här undantaget: `TypeError: Argument 2 passed to Magento\CustomerCustomAttributes\Model\Plugin\ProcessCustomerShippingAddressCustomAttributes::beforeSaveAddressInformation() must be of the type string, null given`.

<!-- AC-7886 -->

* Berättigande till fri frakt beräknas nu som förväntat när villkoret **[!UICONTROL Subtotal (Excl. Tax)]** för kundvagnsregler tillämpas. [GitHub-36760](https://github.com/magento/magento2/issues/36760)
...
* Exporterade produkter har nu rätt `manage_stock`-värde när värdet `use_config_manage_stock` är lika med `1`. Tidigare var det här standardvärdet felaktigt.
...
* Säljare kan nu lägga till en GIF-bild i en produkts bildgalleri från administratören. Tidigare visade Adobe Commerce det här felet: `imagecolorsforindex(): Argument #2 ($color) is out of range`.
...
* `POST V1/order/notify-orders-are-ready-for-pickup` Skickar nu ett e-postmeddelande som varnar kunderna om att en order är redo för upphämtning först när ordern är klar. E-post utlöses nu endast när begäran har uppfyllts utan undantag.
...
* Omstrukturerad kod i hela kodbasen som skapades när kodningspraxis dikterade att alla cacheavsnitt som ingick i `app/etc/env.php` filen skulle innehålla en frontend-sektion. Denna omfaktorisering åtgärdar potentiella problem vid uppgradering av Commerce före 2.4.4-distributioner. [GitHub-38363](https://github.com/magento/magento2/issues/38363)
...
* Ändrade namnet på variabeln `reponseBody` till `responseBody` i `app/code/Magento/CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`.[GitHub-38093](https://github.com/magento/magento2/issues/38093)

<!-- AC-9221 -->

* Åtgärdade avrundningsfel för delsummor som uppstod i transaktioner för produkter för vilka decimalkvantitet har aktiverats. [GitHub-37817](https://github.com/magento/magento2/issues/37817)

<!-- ACP2E-2127 -->

* Datumfiltret för rutnätet för Admin-kund fungerar nu som förväntat enligt det angivna språket.
...
* Reviderade felmeddelanden som utlöses av ogiltig XML-konfiguration för att vara mer informativa. [GitHub-37788](https://github.com/magento/magento2/issues/37788)
...
* Omstrukturerad inaktuell kod som är relaterad till skapandet av dynamiska egenskaper i hela kodbasen.
...
* Emuleringen startar nu under `send()` samtal när emuleringen har slutförts under `getInfoBlockHtml()` samtalet och inga fel loggas. Tidigare var filen `system.log` översvämmad med följande fel: `main.ERROR: Environment emulation nesting is not allowed`. [GitHub-35603](https://github.com/magento/magento2/issues/35603)

<!-- ACP2E-1612 -->

* Du kan nu använda `updateCartItems` mutationen för att uppdatera ett presentkort i en kundvagn. Tidigare kunde endast kortbeloppet uppdateras i kundvagnen av denna mutation.

<!-- ACP2E-1669 -->

* Presentinslagning ingår nu som förväntat i en offert baserad på sammanslagna varukorgar. Tidigare sammanfogades presentmeddelanden, men inte radbrytningar.

<!-- AC-7530 -->

* Mutationen `addProductsToCart` lägger nu till ett presentkort i en kundvagn. Tidigare förväntades presentkortets mottagarnamn, men avsändarens namn returnerades i stället. Adobe Commerce utlöste det här felet: `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* Hantering av adressfel för GraphQL-bibliotekskomponenter fungerar nu som förväntat.

<!-- AC-9058 -->

* Frågan `products` returnerar nu all relevant prisinformation för skyltfönster när dynamisk prissättning är inställd på nej och rabatter tillämpas. Tidigare returnerades inte prisintervallet för produkter. [GitHub-35649](https://github.com/magento/magento2/issues/35649)

<!-- ACP2E-1744 -->

* GraphQL-transaktionsnamn innehåller nu endast frågenamn på den översta nivån i New Relic-loggarna. Tidigare innehöll transaktionsnamn även sekundära frågenamn.
...
* Problem med prisindexerarens prestanda har åtgärdats. Indexerarens prestanda försämras inte längre gradvis. Indexeraren har omstrukturerats för att öka indexerarens bearbetningshastighet genom att ändra hur temporära tabeller skapas. Dessa temporära tabeller återskapas nu i stället för att åtgärdas av mer tidskrävande DELETE-instruktioner.

<!-- ACP2E-2033 -->

* Prisindexerarens prestanda har förbättrats. Ett indextips har lagts till i prisindexeraren som förbättrar körningstiderna för MySQL-frågor som utlöses under indexeringen.
...
* Filtreringen av produktmetadatabeskrivningen när produkten skapas från administratören har förbättrats. Tidigare, om Page Builder var aktiverat, genererades beskrivningen av produktmetadata baserat på standardmasken` {{name}} {{description}}` . Standardmetabeskrivningsmasken för produkten består nu av `{{name}}`. [GitHub-36107](https://github.com/magento/magento2/issues/36107)
...
* Commerce hanterar nu 502-fel från PayPal korrekt genom att förhindra att order skapas när det här felet inträffar. Tidigare skapades ordrar fortfarande trots ett 502-fel från PayPal, vilket ledde till inkonsekvenser mellan handelsposter och PayPals.

<!-- AC-9142 -->

* PayPal Express Checkout-transaktioner inkluderar nu underordnade produkt-SKU:er och fullständiga produkttitlar. Filen `Magento/Paypal/Model/Cart.php` samlar nu in underordnade produkt-SKU:er med deras överordnade produkttitel för att identifiera produkttiteln med den underordnade SKU:n. [GitHub-36808](https://github.com/magento/magento2/issues/36808)

<!-- ACP2E-1964 -->

* Konfigurationsinställningen **[!UICONTROL Card Security Code Does Not Match]** bearbetas nu som förväntat under Payflow Pro-betalningsprocessen. Tidigare slutfördes aldrig den här utvärderingsprocessen, vilket blockerade utcheckningen.
...
* E-postmeddelanden om förfallodatum för belöningspoäng skickas inte längre efter att poängen har löpt ut. Uppgifterna och `magento_reward_balance_warning_notification` `magento_reward_expire_points` cron-uppgifterna har omstrukturerats för att öka flexibiliteten och kapaciteten för att skicka e-post. Tidigare kunde handlare inte skicka många e-postmeddelanden som genereras av dessa cron-uppgifter på kort tid. Detta kunde ha orsakat förseningar i leveransen av tidskänsliga meddelanden.

<!-- ACP2E-2283 -->

* Utgångna belöningspoäng dras nu av från en kunds nuvarande belöningspoängsaldo som förväntat.
...
* Förbättrade meddelanden om fel som uppstår under mallåtergivningen. Commerce visar nu ett informativt meddelande. Tidigare visade Commerce HTML-läckor när ett PHP-fel inträffade.
...
* Ordningen på produktattributtyperna Rullgardinsmeny matchar nu ordningen på bilderna på den konfigurerbara produktsidan. [GitHub-37285](https://github.com/magento/magento2/issues/37285)

<!-- ACP2E-1969 -->

* Systemmeddelanden laddas nu snabbare. Endast de meddelanden som visas laddas. Tidigare lästes alla meddelanden in från databasen, vilket var tidskrävande.
...
* Nyskapade CMS-sidor med en underordnad nod ingår nu i marknadsföringsfilterrutnätet (**[!UICONTROL Marketing]** > **[!UICONTROL URL rewrites]**). Tidigare skapades inte URL-omskrivningar för dessa CMS-sidor.

<!-- ACP2E-1737 -->

* Omskrivningar av kategori-URL:er finns nu i CSV-filen vid import av en ny kategori.

<!-- ACP2E-1668 -->

* CMS-sidor för flera butiker utan URL-omskrivningar vidarebefordras nu som förväntat. Tidigare omdirigerades begärda CMS-sidor för flera butiker utan URL-omskrivningar till butikens startsida, men de ursprungliga CMS-sidorna dirigerades inte.
...
* Du kan nu köra en auktoriserad asynkron bulk-API-begäran för att skapa kunder med ett icke-standard grupp-ID. Tidigare återställer dessa begäranden grupp-ID:t till standardvärdet.
...
* Commerce visar nu ett meddelande som anger att en artikel redan har lagts till i en önskelista när en kund försöker lägga till samma artikel i en önskelista flera gånger. Tidigare under dessa förhållanden ökade Commerce produktkvantiteten i önskelistan.

<!-- AC-8610 -->

* Adobe Commerce lägger nu till produkter med nödvändiga överförda anpassningsbara alternativfiler i önskelistan. Tidigare lade Commerce till två produkter på önskelistan, inte en. [GitHub-37437](https://github.com/magento/magento2/issues/37437)
