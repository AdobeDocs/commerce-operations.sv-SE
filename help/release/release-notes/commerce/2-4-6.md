...
## 2.4.6-beta1-testmiljö

Adobe Commerce 2.4.6-beta1 har testats mot följande komponentversioner:
...
Den här versionen innehåller åtta säkerhetskorrigeringar och plattformssäkerhetsförbättringar. Dessa säkerhetskorrigeringar har säkerhetskopierats till Adobe Commerce 2.4.5-p1 och Adobe Commerce 2.4.4-p3.
...
PHP 8.1 stöds fortfarande till fullo. Adobe Commerce 2.4.6 GA har stöd för PHP 8.2. Fördröjningen med att släppa PHP 8.2 har dock förhindrat oss från att inkludera stöd för den i denna betaversion. <!-- AC-6598 -->

Stöd för PHP 7.4 har tagits bort från den här versionen. Du kan inte köra Adobe Commerce på PHP 7.4.

Adobe Commerce 2.4.6-beta1 har nu stöd för:
...
* MariaDB 10.6 (LST-version). Den här versionen är fortfarande kompatibel med MariaDB 10.4, men vi rekommenderar att du uppgraderar till MariaDB 10.6. <!-- AC-6921 -->
...
Vi åtgärdar hundratals problem i Adobe Commerce 2.4.6 Core-koden. En delmängd av de åtgärdade problemen i Adobe Commerce 2.4.6-beta1 beskrivs nedan.
...
* Kör `bin/magento set:di:com -vvv` (eller med `-v` eller `-vv` alternativ) returnerar nu ett informativt meddelande om felet. [GitHub-35391](https://github.com/magento/magento2/issues/35391)
...
* The `composer install` brödtext skrivs inte längre över `.gitignore` filer. [GitHub-32888](https://github.com/magento/magento2/issues/32888)

<!-- AC-6671-->

* Körs `php bin/magento staticdata:remove` Att ta bort exempeldata efter uppgraderingen misslyckas inte längre med det här felet: `Undefined constant "MagentoHackathon\Composer\Magento\Deploystrategy\GLOB_BRACE`. [GitHub-36062](https://github.com/magento/magento2/issues/36062)
...
* The `bin/magento app:config:import`-kommandot importerar nu innehållet i en `config.php` -fil. Tidigare misslyckades importen med följande fel: `Import failed: Notice: Trying to access array offset on value of type null in [..]/vendor/magento/module-config/Model/Config/Backend/Currency/Cron.php on line 66`. [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* Köpare kan inte längre kringgå reCAPTCHA-validering när de gör en beställning med Braintree när reCAPTCHA är aktiverat.

<!-- BUNDLE-3185-->

* ReCAPTCHA-validering vid utcheckning av order som skickats till flera destinationer fungerar nu som förväntat.
...
* SEO-vänliga URL:er genereras nu för produkter vars `url_key` attributet åsidosätts på butiksvynivå.
