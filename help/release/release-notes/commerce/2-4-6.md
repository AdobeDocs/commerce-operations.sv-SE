...
Adobe Commerce 2.4.6 har nu stöd för PHP 8.2. PHP 8.1 stöds fortfarande fullt ut, men stödet för PHP 7.4 har tagits bort. Det innehåller betydande prestanda- och skalbarhetsförbättringar. GraphQL-åtgärder för gruppvaruåtgärder och kategoriträdåtergivningssvar har optimerats.
...
## 2.4.6-beta2-testmiljö

Adobe Commerce 2.4.6-beta2 har testats mot följande komponentversioner:
...
Adobe Commerce 2.4.6-beta2 har nu stöd för PHP 8.2. PHP 8.1 stöds fortfarande till fullo.  <!-- AC-6598 -->

Stöd för PHP 7.4 har tagits bort från den här versionen. Du kan inte köra Adobe Commerce på PHP 7.4.

Adobe Commerce 2.4.6 beta2 har nu stöd för:

* Composer 2.2.x <!-- AC-2839 -->

* Redis 7.0.x Även om kompatibiliteten med Redis 6.2 kvarstår rekommenderar vi att du installerar den här versionen med Redis 7.0.x eftersom Redis 6.2 kommer att ha upphört i april 2024. <!-- AC-6606 -->
...
Funktionen Inköpsorder för företag visas nu helt och hållet av GraphQL API. Den här versionen innehåller två GraphQL-scheman:
...
**Förbättrad svarstid vid frågor i kategorier med kategoribehörigheter aktiverade**. Svarstiderna har förbättrats vid frågor om produkter i kategorier där kategoribehörigheter är aktiverade och som innehåller många kundgrupper eller delade kataloger. <!-- B2B-2179-->

**Optimerade prestanda för kategoriträdrendering i GraphQL**. The `categoryList` frågan har optimerats för att stödja kunder med ett stort antal kategorier. Handlare som använder den här frågan för att söka i mer än 1 500 kategorier med upp till fem nivåer av kapslade underkategorier kommer att märka en prestandaförbättring.
<!-- B2B-2484-->
...
Se [Utvecklarhandbok för GraphQL](https://developer.adobe.com/commerce/webapi/graphql/) för mer information om dessa förbättringar.
...
Vi åtgärdar hundratals problem i Adobe Commerce 2.4.6 Core-koden. En delmängd av de åtgärdade problemen i Adobe Commerce 2.4.6-beta2 beskrivs nedan.
...
* GraphQL frågesvar innehåller nu information om de underordnade produkterna i `product` i `options` -block.
...
* Antalet kategoriobjekt är nu korrekt efter filtrering på katalogsidan när **Visa slut på lager** är aktiverat. Tidigare var antalet butiksprodukter som inte fanns i lager felaktigt.
...
* Kunden `order` frågan returnerar nu order från alla angivna webbplatser, butiker och butiksvyer som förväntat. Den här korrigeringen innehåller skapande av ett nytt begärandeargument.
...
* Prestandan för sidinläsning har förbättrats när produktlistan för katalogwidgeten läses in. Tidigare lästes icke-globala attribut in direkt från databasen.
