...
---
# bin/magento (Adobe Commerce lokal 2.4.9-beta1)
...
Referensen innehåller 147 kommandon som är tillgängliga via kommandoradsverktyget `bin/magento`.
...
Cache för rensa handelsomfattningar för betalningstjänster
...
Ange om användarens HTML-innehållsvalidering ska framtvingas eller om en varning ska visas i stället
...
Inkludera indexkontroll. Varning: kan orsaka prestandaförsämring. [true\|false]
...
Visar en lista över tillgängliga dataomkrypterare.
...
Ändra krypteringsnyckeln i filen env.php.
...
Returnerar information om en händelseprovider
...
ID:t för en händelseprovider. När det här alternativet inte används returneras information för händelseprovideruppsättningen i systemkonfigurationen.
...
ID:t för en händelseprovider. När du anger det här alternativet filtreras händelseregistreringsdata för ditt App Builder-projekt efter det angivna ID:t.
...
Listan med regler för händelseprenumerationen, där varje regel formateras som&quot;field\|operator\|value&quot;. Om du vill använda det här alternativet måste du även ange det överordnade alternativet.
...
Händelseprovidern som händelser levereras till
...
Visar indexerarens Dimension-läge
...
Skapa en distributionsmarkör i New Relic (stöder både v2 REST och NerdGraph)

### Argument

#### `message`

Distribuera meddelande/beskrivning
...
Git-implementeringshash för den här distributionen (endast NerdGraph)

- Accepterar ett värde

#### `--deep-link`, `-d`

Djuplänk till distributionsinformation (endast NerdGraph)

- Accepterar ett värde

#### `--group-id`, `-g`

Grupp-ID för organisering av distributioner (endast NerdGraph)
...
Initiera ett nytt projekt för handlaren som konfigurerats i tjänstkopplingen
...
Visar projektinformation för handlaren som konfigurerats i tjänstkopplingen
...
Synkronisera delvis igen med en lista över angivna identifierare. (Detta alternativ gäller produkterna, åsidosättningar av produkter och priser)

- Kräver ett värde

#### `--id-type`

Typ av identifierare för partiell omsynkronisering (t.ex. sku, productId)
...
Valkey-databasnummer

- Kräver ett värde

#### `--session-save-valkey-compression-threshold`

Tröskelvärde för Valkey-komprimering

- Kräver ett värde

#### `--session-save-valkey-compression-lib`

Komprimeringsbibliotek för Valkey. Värden: gzip (standard), lzf, lz4, snappy

- Kräver ett värde

#### `--session-save-valkey-log-level`

Värdets loggnivå. Värden: 0 (minst utförlig) till 7 (mest utförlig)
...
Valkey inaktiverar låsning. Värden: false (standard), true

- Kräver ett värde

#### `--session-save-valkey-min-lifetime`

Livstid för min valnyckelsession, i sekunder

- Kräver ett värde

#### `--session-save-valkey-max-lifetime`

Högsta livstid för Valkey-sessioner, i sekunder
...
Valkey Sentinel-servrar, kommaseparerade

- Kräver ett värde

#### `--session-save-valkey-sentinel-verify-master`

Valkey Sentinel verify master. Värden: false (standard), true

- Kräver ett värde

#### `--session-save-valkey-sentinel-connect-retries`

Valkey Sentinel-anslutningsförsök.
...
Serialisering som ska användas (binärt är 70 % snabbare, 58 % mindre än PHP-serialisering)
...
Den URL som butiken ska vara tillgänglig på. Inaktuellt, använd config:set med sökvägen web/unsecure/base_url

- Kräver ett värde

#### `--language`

Standardspråkkod. Inaktuell, använd config:set med sökvägen general/locale/code

- Kräver ett värde

#### `--timezone`

Standardkod för tidszon. Inaktuell, använd config:set med sökvägen general/locale/timezone
...
Använd omskrivningar. Inaktuellt, använd config:set med sökvägen web/seo/use_rewrites

- Kräver ett värde

#### `--use-secure`

Använd säkra URL:er. Aktivera bara det här alternativet om SSL är tillgängligt. Inaktuellt, använd config:set med sökvägen web/secure/use_in_front

- Kräver ett värde

#### `--base-url-secure`

Bas-URL för SSL-anslutning. Inaktuellt, använd config:set med sökvägen web/secure/base_url

- Kräver ett värde

#### `--use-secure-admin`

Kör administratörsgränssnitt med SSL. Inaktuellt, använd config:set med sökvägen web/secure/use_in_adminhtml

- Kräver ett värde

#### `--admin-use-security-key`

Om en säkerhetsnyckel ska användas i Magento Admin URL:er och formulär. Inaktuell, använd config:set med sökvägadmin/security/use_form_key
...
Återställer Magento Application-kodbas, media och databas
...
Uppgraderar Magento-programmet, databasdata och schema
