...
---
# Adobe Commerce åtgärdade problem (v2.4.9-beta1)

## Åtgärdade problem i v2.4.9-beta1

Vi har åtgärdat 560 problem i kärnkoden för Adobe Commerce 2.4.9-beta1. En deluppsättning av de åtgärdade problemen som ingår i den här versionen beskrivs nedan.
...
Villkoret&quot;Produkten visades&quot; i kundsegmentet är nu föråldrat.
Tidigare kunde det orsaka platsavbrott om det här villkoret användes på grund av tunga MySQL-frågor. Villkoret är nu markerat som inaktuellt och stöds inte.
...
#### Varningsmeddelande under funktionen Skicka inbjudan

Korrigerade problemet där varningsmeddelandet&quot;Högsta antal tillåtna X-e-postadresser&quot; inte visades när e-postfält lades till på sidan Skicka inbjudningar om inställningen&quot;Tillåt kunder att lägga till anpassat meddelande till e-postinbjudan&quot; inaktiverades.
Tidigare visades varningen endast när anpassade meddelanden var aktiverade, vilket skapade en inkonsekvent användarupplevelse. Nu visas den maximala e-postbegränsningsvarningen konsekvent oavsett inställningen för anpassad meddelandekonfiguration.
...
#### Produktavbildningsändringar har inte loggats i åtgärdsloggen

Korrigerade ett problem där produktavbildningar som överförs och tas bort inte spårades i administratörens åtgärdsloggar. Tidigare, när administratörer lade till nya bilder till en produkt eller tog bort befintliga bilder från produktens mediegalleri, spelades dessa ändringar inte in i loggningssystemet. Endast ändringar av bildroller (som att tilldela en bild som huvudproduktbild, miniatyrbild eller liten bild) loggades. Nu loggas alla ändringar i mediegalleriet, inklusive bildtillägg och borttagningar, korrekt i administratörens åtgärdsloggar, vilket ger fullständig åtkomsthistorik för produktbildhanteringsaktiviteter.
...
#### Användarrapporten för administratörsåtgärdsloggen visar inte information om vilket filter som användes när filter tillämpades

Filterparametrar loggades inte i administratörsaktivitetsrapporten innan korrigeringen utfördes. Efter korrigeringen loggas alla data i begäran.
...
Korrigerade ett TypeError-fel som uppstod när grupperade produkter lades till i en rekvisitionslista med kategoribehörigheter aktiverade genom att se till att produktalternativ hanteras på ett säkert sätt som arrayer, vilket gör att alla produkttyper kan läggas till utan undantag.
...
#### Borttagning av listrutealternativ fungerar inte när attribut skapas på produktsidan
...
#### Knappen&quot;Lägg till nytt attribut&quot; visas för en begränsad administratörsanvändare när en konfigurerbar produkt skapas

Knappen&quot;Lägg till nytt attribut&quot; visas nu bara för den allmänna administratörsanvändaren när en konfigurerbar produkt skapas.
Knappen Lägg till nytt attribut visades tidigare för begränsad administratörsanvändare
...
#### Relaterade produkter tas bort automatiskt

Relaterade produkter som matchas av målregler förblir nu korrekt associerade under indexeringsprocessen
...
#### [Cloud] Retursida för beställning för produktpaketrad avmarkeras automatiskt

Tidigare hade vi för paketprodukten Ship Together Return i administratörspanelens rutnätsvy ett alternativ som skapade förvirring för alternativet Ship Together. När den här korrigeringen har tillämpats finns det inget mer alternativ för att välja artiklar för paketleveranser.
...
#### Administratör med behörighet på webbplatsnivå kan inte redigera dynamiskt block

Nu kan administratörsanvändare med behörigheter för webbplatsens omfång redigera innehållet i banners i tillgängliga lagrade vyer.
...
Belopp för leveransetiketter matchar nu frakt- och hanteringspriser.
Tidigare stämde beloppet inte med frakt- och expeditionspriset när du laddade ned en etikett.
AC-14560
...
#### Fake Modules Kräver dev/ Directory i tilläggskataloger
...
#### [CLOUD] DateTimeZone-fel när produktändringar sparas via REST API

Före korrigeringen skulle en REST API-begäran för produktuppdatering generera ett fel om det inte fanns någon butik med koden &quot;default&quot;. Efter korrigeringen kan produktuppdateringsbegäran köras oavsett om det finns ett standardarkiv eller inte.
...
#### Stock-status återställdes felaktigt till i lager med kvantiteten efter SKU-ändring under mellanlagringsuppdatering

SKU-ändringar är nu förbjudna för produkter med en aktiv schemalagd uppdatering. Sparandet misslyckas med ett tydligt fel och admin-SKU-fältet inaktiveras under aktiva uppdateringar. Detta förhindrar inkonsekvenser i MSI-lagret som orsakas av SKU-ändringar under mellanlagringsåterställningar.
...
Ett problem har korrigerats där det inte gick att skapa rekvisitionslistor för produkter som tilldelats en ny webbplats och källa när Lägg till butikskod i URL:er var aktiverat. Problemet uppstod eftersom lagringskoden rensades från API-begäran, vilket orsakade ett otillåtet fel. När korrigeringen är klar bevaras rätt butikskontext och rekvisitionslistor skapas.
...
#### Butiksspecifik avsändare används inte för presentkortsmeddelanden

Tidigare, när en e-postmall skickas för ett presentkort efter att en faktura har skapats från en annan butik, skulle ägarens namn i inställningarna för administratörskonfiguration inte återspeglas i e-posthuvudena när kunden fick e-postmeddelandet. När den här korrigeringen har tillämpats inkluderar e-postrubrikerna nu den aktuella butiksägarens e-postinformation.
...
#### Det går inte att lämna värdet tomt för &quot;Belöna vitpunkter vid&quot; - Sparat

Adobe Commerce tillåter nu handlare att lämna fältet Kap belöningspoäng till tomt samtidigt som de fortfarande anger ett värde för Inlösningströskeln för belöningspoäng. När du konfigurerade belöningspunkter under Lager > Konfiguration > Kunder > Belöningspunkter och anger ett positivt tal för Inlösningströskeln för belöningspoäng och lämnar Kap-saldo för belöningspoäng tom utlöste valideringsfelet:&quot;Kap-saldo för belöningspoäng\&quot; är ogiltigt. Balansen måste vara ett positivt tal eller lämnas tom. Verifiera och försök igen.&quot;, vilket förhindrar handlare från att spara konfigurationen utan ett tak. ACP2E-3977
...
#### [Cloud] php-fpm respekterar inte max_execution_time

Distributionskonfigurationen läses nu in en gång i en enda begäran.
...
Tidigare uppstod felet&quot;Ett fel uppstod när programmet kördes om du klickade på&quot;Skriv ut&quot;. Mer information finns i undantagsloggen.&quot;
AC-14711
...
#### Inkonsekvent sorteringsordning i kundsegment gör att X-Magento-Vary-cookie genereras om hela tiden på vissa sidor

X-Magento-Vary-cookie anger nu en gång på produktsidan, och tidigare med vissa konfigurationer av kundsegment ställer cookien in flera gånger under PDP-inläsning
...
#### Efter korrigeringen för ACP2E-2926 matchas kundsegment vid varje utcheckningsbegäran, vilket orsakar onödig bearbetning

Funktionen för kundsegment har nu en cachningsmekanism som förbättrar prestandan.
...
Beställningstillståndstilldelning fungerar nu som förväntat.
Tidigare kunde läget &quot;Bearbetning&quot; försvinna från listrutan när du tilldelat en status, vilket gjorde det omöjligt att tilldela om.
AC-15010
...
#### Problem med förhandsgranskningsläget för erbjudanden där tillämpade kuponger försvinner kort efter att de har tillämpats.

Före korrigeringen kunde verifikationskoderna inte användas korrekt i förhandsgranskningsläget för mellanlagring. Efter korrigeringen tillämpas verifikationskoderna korrekt på utcheckningssidan.
...
Magento uppdaterar nu korrekt momssummor när presenter tas bort via mutationen setGiftOptionsOnCart i GraphQL. Tidigare när ett alternativ för radbrytning hade valts och sedan tagits bort genom att&quot;presentWrappingId&quot; skickades: null i mutationsindata, uppdaterades inte momsbeloppet i offerten och Magento fortsatte att inkludera moms för radbyten i kundvagnen, även om ingen radbrytning tillämpades.
...
#### [Molnbaserad tjänst] CNS-byggfel - 2.4.9-beta1- integrering
...
#### PHPUnit-konfigurationsfilen matchar inte mellan PR- och Composer-byggen
...
#### [Enhetstest] Magento\GiftCardImportExport\Test\Unit\Model\Import\Product\Type\GiftCardTest::testIsRowValid

Korrigering tillhandahölls för ett slumpmässigt underkänt enhetstest
...
#### GUID sparas inte som skyddat format

Det finns ingen beskrivning.

_AC-15809_

#### Kompatibilitetsverktyg för uppgradering med fel allvarliga problem
