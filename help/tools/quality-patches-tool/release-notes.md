...
* **ACP2E-4416** (för Adobe Commerce >=2.4.4 &lt;2.4.9) - Korrigerar problemet där kundbelöningspunkter inte initieras när de skapas i Admin.
* **ACP2E-4419** (för Adobe Commerce >=2.4.6 &lt;2.4.8) - Korrigerar problemet där presentkort inte används korrekt vid utcheckning efter lyckad reCAPTCHA v2-validering (&#39;Jag är inte en robot&#39;) på butiken.
* **ACP2E-4431** (för Adobe Commerce >=2.4.4 &lt;2.4.9) - Korrigerar problemet där relaterade produkter som matchar målreglerna tas bort under omindexeringsprocessen.
* **ACP2E-4448** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där konfigurationsändringar som gjorts under Redis-avbrott inte återspeglas efter Redis-återställning, vilket gör att inaktuella värden kvarstår.
* **ACP2E-4452** (för Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Korrigerar problemet där produktpriser på snabbordersidan inkluderar moms oavsett momsvisningskonfiguration.
* **ACP2E-4456** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar ett fel där det inte går att avbryta en order som använder en GraphQL-mutation och inte överför en order som betalats helt med presentkort till statusen Stängt.
* **ACP2E-4507** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där lösenordsalternativskonfigurationen inte tillämpas för begäranden om återställning av kundlösenord som gjorts via GraphQL-mutationer.
* **ACP2E-4513** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där utgångna CAPTCHA-bilder inte tas bort från systemet.
* **ACP2E-4522** (för Adobe Commerce >=2.4.7 &lt;2.4.9) - Korrigerar problemet där ett återkommande dubblettnyckelfel inträffar i tabellen quote_coupons när flera sparade kundvagnsbegäranden körs samtidigt.
* **ACP2E-4528** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet med stadsvalidering i kundadresser, som nu tillåter ett snedstreck (/) och avvisar ogiltiga tecken som !, &quot;, # och ?.
* **ACP2E-4535** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.9) - Korrigerar ett fel där om du skickar formuläret för glömt lösenord förstörs eller genereras om sessionen (PHPSESSID-ändringar) och gästvagnen rensas.
* **ACP2E-4540** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.9) - Korrigerar problemet där Fotoramabiblioteket inte lästes in korrekt, vilket innebär att endast den första bifogade bilden visas.
* **ACP2E-4555** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där moderna telefonnummer som innehåller &quot;.&quot; eller &quot;/&quot; inte har validerats korrekt.
* **ACP2E-4565** (för Adobe Commerce, B2B >=1.5.0 &lt;1.5.3) - Korrigerar problemet där företagsfrågan returnerar &quot;Den aktuella kunden är inte auktoriserad&quot; när rubriken X-Adobe-Company används.
* **ACP2E-4591** (för Adobe Commerce >=2.4.4 &lt;2.4.8) - Korrigerar problemet där kundsegment baserat på orderantal, som&quot;förstagångsköpare&quot;, inte uppdaterades när beställningar gjordes via REST API.
* **ACP2E-4609** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.9) - Korrigerar problemet där sidan Mina offerter inte visar några citattecken när vissa citattecken innehåller borttagna produkter.
* **ACP2E-4613** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.9) - Korrigerar problemet där stora mediakatalogstrukturer orsakade långsamma gettree-svar, vilket ledde till att katalogträdet för utökade media Gallery lästes in.
* **ACP2E-4628** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där import av kunder med e-postadresser i versaler resulterar i ett odefinierat matrisnyckelfel när kontodelning är inställt på Global.
* **ACP2E-4665** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där underordnade produkter för konfigurerbara produkter som innehåller videor i produktgallerierna inte listas när de begärs via REST API.
...
* **ACSD-63687** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.7) - Korrigerar problemet där felaktiga priser visas eftersom det inte går att rensa Redis-cachen.
* **ACSD-68341** (för Adobe Commerce >=2.4.4 &lt;2.4.9) - Korrigerar problemet när X-Magento-Vary-cookie ställs in flera gånger under PDP-inläsning när flera kundsegment skapas i butiken.
* **ACSD-68537** (för Adobe Commerce >=2.4.8 &lt;2.4.9) - Korrigerar problemet där utcheckningsprestanda försämrades när antalet kundsegment ökade.
* **ACSD-68664** (för Adobe Commerce >=2.4.6 &lt;2.4.9) - Korrigerar problemet där den schemalagda uppdateringsförhandsvisningen avbryts vid försök att förhandsgranska innehåll för butiker med anpassade domäner.
* **ACSD-68759** (för Adobe Commerce och Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.9) - Korrigerar det problem där det inte går att skapa ett kundkonto när det arabiska språket används och födelsedatumet (DOB) är inställt på att visas på butiken.
* **ACSD-68892** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet när cachelagrade sidor inte lagras eller hanteras korrekt från snabbcachen, vilket resulterar i inkonsekvent cachelagring och minskad prestanda.
* **ACSD-69016** (för Adobe Commerce >=2.4.7 &lt;2.4.9) - Korrigerar problemet när specialpriset inte gäller för webbplatser som skapats i olika tidszoner.
* **ACSD-69020** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.8) - Korrigerar problemet där en konfigurerbar produkt automatiskt inkluderas i produktkarusellerna för PageBuilder om någon av dess underordnade produkter uppfyller filtervillkoren.
* **ACSD-69237** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där antalet poster som kan bearbetas och infogas via seriejobben sales_async_insert är begränsat till 100 per körning.
* **ACSD-69311** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet med felaktig momsberäkning i kreditnotor när en partiell återbetalning skapas från en faktura, om en tidigare kreditnota skapades från ordervysidan.
* **ACSD-69351** (för Adobe Commerce >=2.4.4 &lt;2.4.9) - Korrigerar problemet där presentkortsaldon och utgångsdatum inte visas i enlighet med det tilldelade webbplatsomfånget.
* **ACSD-69494** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemen med asynkrona återbetalningsåtgärder där återbetalningsbegäranden med parametern &quot;is_online&quot; inte behandlas korrekt.
* Uppdaterade versioner: **ACSD-67250**
* Ersatta korrigeringsfiler: **ACSD-62629**, **ACSD-66157**
* Inaktuella korrigeringsfiler: **ACSD-66157**

## v1.1.76 {#v1-1-76}

* **ACSD-67091** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar det maximala skrivstorleksfelet för att säkerställa rensning av katalogregelns produktindex genom att implementera två raderingsstrategier som baseras på datavolym.
* **ACSD-67370** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.9) - Korrigerar flera problem där felaktiga priser visades för paketprodukter på PDP/PLP och kundvagnssidan för butiker med flera valutor.
* **ACSD-68410** (för Adobe Commerce, B2B >=1.3.3 &lt;1.5.3) - Korrigerar ett problem där en order för en överlåtbar offert felaktigt lägger till eller sammanfogar ytterligare kundvagnsrader i offerten. Produkterna läggs nu korrekt till i kundvagnen efter att du lämnat det sista steget i utcheckningen av en överlåtbar offert.
* **ACSD-69086** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.8) - Korrigerar det problem där cron-jobbet inte kan rensa ändringstabeller, vilket orsakar att Galera Cluster kraschar när stora mängder data hanteras.
* **ACSD-69115** (för Adobe Commerce >=2.4.4 &lt;2.4.9) - Korrigerar ett problem där kundvagnsfel inte visades för administratörsanvändaren när kundvagnen hanterades för en kund som tilldelats en icke-standardwebbplats.
* **ACSD-69129** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.7 || >=2.4.8 &lt;2.4.9) - Korrigerar ett fel där borttagning av standardbaswebbplatsen och användning av den sekundära webbplatsen som standard resulterade i ett fel när nivåpriset för den sekundära webbplatsen skulle uppdateras via REST API.
* **ACSD-69203** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar ett fel där widgeten Produktlista returnerade felaktiga resultat när flera kategorier listades i kategorivillkoret.
* **ACSD-69261** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar ett problem där en kundvagnsregelkupong som konfigurerats för engångsbruk återanvändes flera gånger på grund av felaktig hantering av attributet `times_used` i scenarier med partiella fakturor och återstående antal.
* **ACSD-69308** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar ett fel där katalogprisreglerna inte tillämpades när `special_price` bara angavs på webbplatsnivå (inte på Alla butiksvyer). Efter korrigeringen tillämpas katalogprisreglerna korrekt genom att kontrollera webbplatsens standardbutik först.
* **ACSD-69319** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.9) - Korrigerar ett problem där paketpriserna inte indexerades korrekt när underordnade produkter fanns i lager under anpassade källor.
* **ACSD-69325** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.9) - Korrigerar ett fel där ändring av SKU-ärendet gjorde att produkten inte fanns på lagret.
* **ACSD-69331** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.9) - Korrigerar ett fel där innehållsskapare i mediegalleriet inte kunde skapa mappar med endast behörigheten `create_folder`. Efter korrigeringen kan de skapa mappar som förväntat.
* **ACSD-69333** (för Adobe Commerce >=2.4.7 &lt;2.4.9) - Korrigerar ett fel där SKU-ändringar var tillåtna för produkter med en aktiv schemalagd uppdatering. Efter korrigeringen tillåts inte SKU-ändringar under aktiva uppdateringar. Sparningar misslyckas med ett tydligt fel och fältet admin-SKU är inaktiverat. Detta förhindrar inkonsekvenser i MSI-lagret som orsakas av SKU-ändringar under mellanlagringsåterställningar.
* **ACSD-69541** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar ett fel där en minskning av en produkts kvantitet i administratören till mindre än vad som redan finns i en kundvagn gjorde det omöjligt att redigera produktkvantiteten i varukorgen via GraphQL.
* Uppdaterade versioner: **ACSD-46541**, **ACSD-53750**, **ACSD-66404**
* Ersatta korrigeringsfiler: **ACSD-66404**, **ACSD-68499**

## v1.1.75 {#v1-1-75}

* **ACSD-68289** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar ett fel där fulltextsökning nu returnerar matchande produkter om minimivillkoret för matchning uppfylls för alla sökbara fält tillsammans, i stället för att villkoret måste uppfyllas av ett enskilt fält.
* **ACSD-68359** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar ett fel där det inte längre går att välja en butik vid utcheckning med [!UICONTROL Pick in Store] på grund av långa URL:er när många produkter finns i vagnen. Tidigare utlöste detta ett *414-fel* som orsakades av alltför långa URL:er som genererades när butiken valdes, vilket hindrade kunderna från att slutföra utcheckningen.
* **ACSD-68451** (för Adobe Commerce, B2B >=1.5.2-p1 &lt;1.5.3) - Korrigerar ett problem för flera webbplatser där en företagsadministratör loggar in på en webbplats, skapar ett icke-relaterat företag på en annan webbplats, men är felaktigt länkat till det icke-närstående företaget.
* **ACSD-68490** (för Adobe Commerce >=2.4.6 &lt;2.4.7) - Korrigerar problemet där knappen [!UICONTROL Add New Attribute] är synlig för en begränsad administratörsanvändare när en konfigurerbar produkt skapas.
* **ACSD-68517** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar ett formuläröverföringsfel på sidorna för katalog- och katalogsökning.
* **ACSD-68573** (för Adobe Commerce >=2.4.5 &lt;2.4.9) - Korrigerar problemet där kategoribehörigheter inte tillämpades korrekt på kundens önskelisteobjekt. Efter korrigeringen visas önskelisteobjekten korrekt och sidnumreras både på webben och i GraphQL.
* **ACSD-68615** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där CLI för kompensation för lagerreservation visade ett undantag om den bearbetade kombinationen hade ett order-ID som saknades.
* **ACSD-68793** (för Adobe Commerce, B2B >=1.5.1 &lt;1.5.3) - Korrigerar ett problem där giltiga produkter felaktigt avvisades när de tilldelades till en delad katalog.
* **ACSD-68925** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar ett fel där svar för GraphQL-begäranden nu är justerade mot GraphQL över HTTP-specifikationer. En 4XX-svarskod returneras när begäran inte kan tolkas, inte är auktoriserad eller stöter på ett allmänt problem. Om begäran tolkas och kan behandlas returneras en svarskod på 200.
* Uppdaterade versioner: **MDVA-19640**, **ACSD-47910**, **ACSD-68040**, **ACSD-62965**
* Ersatta korrigeringsfiler: **ACSD-62577**, **ACSD-68011**
...
* **ACSD-68499** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar ett fel där GraphQL `updateCartItems`-mutationen returnerar ett felaktigt svar när kvantiteter som överskrider det tillgängliga lagret uppdateras, vilket ger upphov till uppblåsta mängder och summor.
* **ACSD-68810** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar ett fel där en order tilldelas en kund som skapats på en annan webbplats, trots **[!UICONTROL Customer Account Sharing]**-konfigurationen.
...
* **ACSD-68040** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar problemet där söksidans prestanda försämras på [!DNL MariaDB] 10.6 och 11.4 med många tidigare sökbegäranden.
...
* **ACSD-66807** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där tabellen `report_viewed_product_index` visar ett felaktigt antal produktsidesvisningar.
* **ACSD-67383** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där **[!UICONTROL Login as Customer]** med två företagsadministratörskonton i samma session orsakar ett *Ingen sådan entitet med cartId*-fel.
...
* **ACSD-67696** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där `media_gallery`-poster inte returneras i produktnoden för kundvagnen för GraphQL efter en cachetömning.
...
* **ACSD-68118** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.9) - Korrigerar problemet där GraphQL-frågan `customerCart` returnerar produktattributvärden som inte återspeglar butikshuvudet, vilket orsakar inkonsekvent lokalisering.
...
* **ACSD-60624** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där **[!UICONTROL Upload Image]** inte fungerar för tomt innehåll i avsnitten [!UICONTROL Image], [!UICONTROL Banner] och [!UICONTROL Slider] i Page Builder.
* **ACSD-67089** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar sidnumreringsproblemet i API:t `inventory/export-stock-salable-qty`, som felaktigt begränsar `total_count` till sidstorleken.
...
* **ACSD-66120** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där [!DNL GraphQL] felaktigt visar rabattprocentsatser och baspriser när katalogpriserna är konfigurerade att inkludera moms.
...
* **ACSD-67166** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där `cataloginventory_stock_status`-frågan körs flera gånger när ett citat läses in på butiken, vilket orsakar redundanta databasanrop.
...
* **ACSD-67686** (för Adobe Commerce och Magento Open Source >=2.4.4-p15 &lt;2.4.5 || >=2.4.5-p14 &lt;2.4.6 || >=2.4.6-p12 &lt;2.4.7) - Korrigerar problemet där ett &lt;ph id=&#39;14 Ett fel (/>) inträffar när en tom [!DNL GraphQL]-begäran skickas.`Syntax Error: Unexpected <EOF>`
* **ACSD-67250** (för Adobe Commerce >=2.4.7-p4 &lt;2.4.8) - Korrigerar problemet där sparåtgärden **[!UICONTROL Shared Catalog]** uppdaterar alla objekt i stället för bara de som påverkas, vilket förbättrar prestandan genom att eliminera onödiga åtgärder.
...
* **ACP2E-3731** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där produktexporter med synligheten **[!UICONTROL Catalog, Search]** felaktigt inkluderar poster från andra butiksvyer i miljöer med flera butiker.
...
* **ACSD-67347** (för Adobe Commerce och Magento Open Source >=2.4.5-p11 &lt;2.4.6) - Korrigerar problemet där beställningen misslyckas med *Det går inte att få ett lås* när kuponger med specialtecken används och fillåsning är aktiverat.
...
* **ACSD-66179** (för Adobe Commerce och Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.9) - Korrigerar problemet där annullering av en faktura som skapats med betalningstypen &quot;Inte hämtad&quot; resulterade i en 404-felsida.
...
* **ACSD-66963** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.8) - Korrigerar problemet där mutationen `estimateTotals` returnerade null för rabatter när en rabattkod tillämpades på en kundvagn som innehåller virtuella produkter.
* **ACSD-67039** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där kundposter inte sparades på grund av valideringen av `rp_token`-systemattributet och diakritikverifieringen tillämpas nu endast på den resulterande kundens e-post.
...
* **ACSD-65935** (för Adobe Commerce >=2.4.4 &lt;2.4.8) - Korrigerar problemet där GraphQL-frågan `customerOrders` returnerade ett internt serverfel när en produkt togs bort.
* **ACSD-66049** (för Adobe Commerce och Magento Open Source >=2.4.5-p3 &lt;2.4.6 || >=2.4.7 &lt;2.4.9) - Korrigerar problemet där icke-engelska butiker visar felaktiga priser på grund av ICU-biblioteksversionen.
...
* **ACSD-66118** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där uppdatering av lagringsvykoden rensar [!UICONTROL Design Configuration]-inställningarna om konfigurationscachen inte uppdateras.
...
* **ACSD-66301** (för Adobe Commerce och Magento Open Source >=2.4.6-p9 &lt;2.4.7 || >=2.4.7-p4 &lt;2.4.8) - Korrigerar problemet där förflyttning av produkter från en order tillbaka till kundvagnen i Admin resulterar i en kvantitetsmatchningsfel.
...
* **AC-14984** (för Adobe Commerce och Magento Open Source >=2.4.6-p10 &lt;2.4.7 || >=2.4.8 &lt;2.4.9) - Korrigerar felet *Ogiltig bildrutetyp 21* på RabbitMit SQL-anslutning.
* **AC-14985** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där e-post inte skickas när den externa `smtp`-servern med TLS är aktiverad.
...
* **ACSD-65775** (för Adobe Commerce och Magento Open Source >=2.4.8 &lt;2.4.9) - Korrigerar problemet där API-ordningsinformationen för [!DNL REST] returnerade felaktiga `base_row_total` - och `row_total`-värden när flera kvantiteter av samma objekt beställdes.
...
* **ACSD-66041** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.9) - Korrigerar problemet där Irland (IE) postkoder inte kunde sökas efter upphämtningsplatser eftersom `CountryID` saknas.
...
* **ACSD-64813** (för Adobe Commerce >=2.4.4 &lt;2.4.9) - Korrigerar problemet där det tar för lång tid eller tar för lång tid att frigöra kategorier från en delad katalog med [!DNL B2B] via [!DNL REST] API.
...
* **ACSD-66093** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där e-postadresser kan anges i gästkundens [!UICONTROL First Name]- och [!UICONTROL Last Name]-fält, vilket resulterar i ogiltiga orderbekräftelsemeddelanden.
* Uppdaterade versioner: **ACSD-51291**
* Ersatta korrigeringsfiler: **ACSD-61522**

## v1.1.64 {#v1-1-64}

* **ACP2E-3838** (för Adobe Commerce och Magento Open Source >=2.4.4-p9 &lt;2.4.4-p13 || >=2.4.5-p8 &lt;2.4.5-p12 || >=2.4.6-p6 &lt;2.4.6-p10 || >=2.4.4 7 &lt;2.4.7-p5) - Korrigerar problemet där fel i [!DNL Page Builder] CORS förhindrar att ändringar sparas i administrationspanelen i produktionsläge.
...
* **ACSD-65127** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där aktivering av JavaScript minification i produktionsläge medför att [!DNL TinyMCE] 6 genererar fel i webbläsarkonsolen, vilket påverkar funktionalitet och användarupplevelse.
...
* **ACSD-63406** (för Adobe Commerce och Magento Open Source >=2.4.4-p9 &lt;2.4.5 || >=2.4.5-p8 &lt;2.4.6 || >=2.4.6-p6 &lt;2.4.8) - Korrigerar problemet där utgångna beständiga citattecken inte rensas av något cron-jobb när &lt;ph id= &quot;211&quot;/> cron job running.`persistent_clear_expired`
* **ACSD-63520** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.8) - Korrigerar problemet där bilder som läggs till via **[!UICONTROL Configurations]** på admin-panelen inte följer den maximala överföringsstorleksgränsen.
...
* **ACSD-65164** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.8) - Korrigerar problemet där felmeddelandet *Vissa av de valda objektsalternativen inte är tillgängliga för tillfället* inträffar när en konfigurerbar produkt med ett enda markerat kryssrutealternativ ändras.
...
* **ACP2E-3705** (för Adobe Commerce >=2.4.7 &lt;2.4.8) - Korrigerar ett fel där körningen av kron i `indexer_update_all_views` misslyckas när `MAGE_INDEXER_THREADS_COUNT` anges.
...
* **ACSD-64112** (för Adobe Commerce >=2.4.5 &lt;2.4.8) - Korrigerar ett fel där kronkörningen av `indexer_update_all_views` misslyckas när `MAGE_INDEXER_THREADS_COUNT` är inställd.
...
* **ACSD-63182** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet när ett fel inträffar när en duplicerad paketprodukt sparas med **[!DNL MSI]** *enabled*.
...
* **ACSD-63329** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där standardvärden för attribut med indatatyperna **[!UICONTROL Date]** eller **[!UICONTROL Date and Time]** inte anges när produkter skapas via [!DNL REST API].
* **ACSD-63572** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.8) - Korrigerar problemet där temporära tabeller för `CatalogRule`-indexeraren inte rensas om indexeringsprocessen avslutas.
...
* **ACSD-59083** (för Adobe Commerce >=2.4.4 &lt;2.5.0) - Korrigerar problemet där vissa databasuppdateringsåtgärder resulterar i ett *Bastabell eller vy som inte hittas* om [!DNL mview]-uppdateringen körs samtidigt.
...
* **ACSD-61895** (för Adobe Commerce >=2.4.4 &lt;2.5.0) - Korrigerar problemet där frågan [!DNL GraphQL] returnerar kategorier med behörigheten **allow** även om rotkategorin inte har behörigheten **allow** .
* **ACSD-62212** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.5.0) - Korrigerar problemet där e-postinnehållet **Forgot Password** inte översätts till butiksvyns språk.
...
* **ACSD-62629** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.5.0) - Korrigerar problemet där en produktlista som används i **[!UICONTROL Widgets]** inte återspeglar kategorivillkoret.
* **ACSD-62635** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.5.0) - Korrigerar problemet där produkter som är relaterade till flera lager inte visas korrekt i [!DNL GraphQL]-produktfrågan.
...
* **ACSD-62708** (för Adobe Commerce och Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6-p2 || >=2.4.6-p8 &lt;2.4.7-p1) - Korrigerar problemet där &lt;ph id=&#39;1 Teckensnittsstorleken 7 i redigeraren i administratören visar *PT* och inte *PX*. [!DNL TinyMCE] Nu kan du även ange teckenstorleken i *PX* i stället för *PT*.
...
* **ACSD-63242** (för Adobe Commerce och Magento Open Source >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) - Korrigerar problemet med låg importhastighet när du lägger till katalogprodukter med fler än 10 000 poster.
...
* **ACSD-62979** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar problemet där fel [!UICONTROL Store ID] i huvudet [!DNL GraphQL] orsakar ett allvarligt minnesfel.
* **ACSD-62971** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där import av Stock-källor med icke-numeriska värden i **quantity** resulterar i att **quantity** ställs in på *0*.
...
* **ACSD-62670** (för Adobe Commerce och Magento Open Source >=2.4.4-p11 &lt;2.4.5 || >=2.4.5-p10 &lt;2.4.6 || >=2.4.6-p8 &lt;2.4.7 || >=2.4.7-p3 &lt;2.4.8) - Korrigerar problemet där [!UICONTROL Products Ordered]-rapportexporten till [!DNL CSV] och [!DNL XML] returnerar ett fel.
...
* **ACSD-62428** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.7) - Korrigerar problemet där `is_out_of_stock` är inställt på ett felaktigt värde i katalogsökindexet när [!DNL SKU] inte är inställt som ett sökbart attribut.
...
* **ACSD-58383** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där en återbetalning skapas via [!DNL REST API] med två identiska begäranden som utförs samtidigt, skapar dubbla kreditnotor.
...
* **ACSD-58566** (för Adobe Commerce >=2.4.6 &lt;2.4.8) - Korrigerar problemet där [!DNL GraphQL] returnerar ett internt serverfel när fältet `created_at` efterfrågas i mutationen `addPurchaseOrderComment`.
...
* **ACSD-58828** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.8) - Korrigerar problemet där valideringsmeddelandet *adress krävs* visas om ett obligatoriskt fält lämnas tomt, tillsammans med valideringsmeddelandet på klientsidan. Verifieringen på serversidan visar inte meddelandet om tomma obligatoriska fält, och verifieringen på klientsidan kommer att hantera felmeddelandet med meddelandet *Detta är ett obligatoriskt fält.*
...
* **ACSD-61348** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där önskelisteobjekt visas via [!DNL GraphQL], men inte på butiken i en miljö med flera webbplatser.
...
* **ACSD-61103** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.7) - Korrigerar problemet där antalet fel i tabellen `customer_entity` inte återställs till noll efter att en kund loggat in via API-slutpunkter.
...
* **ACSD-61200** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där beräkningarna för *[!UICONTROL Total Amount]* och *[!UICONTROL Total Amount Actual]* vid försäljning saknar *[!UICONTROL Discount Tax Compensation Amount]* och *[!UICONTROL Shipping Discount Tax Compensation Amount]*, vilket orsakar avvikelser i försäljningsorderdata.
* **ACSD-61522** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där det går att ange e-postadresser i gästkundens *[!UICONTROL First Name]*- och *[!UICONTROL Last Name]*-fält och skicka ogiltiga orderbekräftelsemeddelanden via e-post.
...
* **ACSD-61133** (för Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.8) - Korrigerar problemet där `sales_clean_quotes` &lt;ph id=&#39;1 27&#39;/> tar bort offerter från ej godkända inköpsorder.[!DNL cron]
...
* **ACSD-61553** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.7) - Korrigerar problemet där **[!UICONTROL Cart Price Rule]**-rabatterna inte beräknas korrekt när flera rabatter med olika prioritet och **[!UICONTROL Maximum Qty Discount is Applied To]** används för produkten.
...
* **ACSD-59865** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar problemet där en [!UICONTROL Cart Price Rule] inte avbryter tidigare tillämpade regler om kvantiteten av produkten i kundvagnen inte är tillräckligt för att reglerna ska tillämpas.
...
* **ACSD-59952** (för Adobe Commerce >=2.4.4 &lt;2.4.8) - Korrigerar problemet när ett fel inträffar när en [!UICONTROL Shared Catalog] skapas med ett grupp-ID som är tilldelat till en befintlig [!UICONTROL Shared Catalog].
* **ACSD-60590** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Förbättrar prestanda för att generera [!UICONTROL Bestsellers Aggregated Daily Reports] för en stor mängd placerade order.
...
* **ACSD-61322** (för Adobe Commerce >=2.4.6 &lt;2.4.8) - Korrigerar problemet där [!UICONTROL Products/Categories] inte har tilldelats [!UICONTROL Shared Catalog] för standardgruppen (General Group) fortfarande ingår i XML-platskartan.
* **ACSD-61366** (för Adobe Commerce och Magento Open Source >=2.4.7 &lt;2.4.8) - Korrigerar problemet där kommandot `setup:static-content:deploy --jobs 4` körs med flera jobb som misslyckades med *Port måste konfigureras inom värdparameterfelet* när porten har angetts för DB-anslutningen.
...
* **ACSD-60234** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där ett felaktigt belopp visas på [!DNL PayPal] när rabatten tillämpas av betalningsmetoden.
...
* **ACSD-60538** (för Adobe Commerce >=2.4.7 &lt;2.4.8) - Korrigerar problemet där en produkt inaktiveras i *[!UICONTROL All Store Views]* och endast aktiveras i specifika butiksvyområden, så visas inte produktattributen korrekt i GraphQL-svaret, vilket gör att produkten inte visas korrekt.
...
* **ACSD-60816** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar problemet där de [!DNL New Relic Browser Monitoring]-skript som injicerats av APM-agenten inte är kompatibla med CSP (Content Security Policy), vilket förhindrar att de körs.
...
* **ACSD-59280** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.5) - Korrigerar felet *Anrop till undefined method ReflectionUnionType::getName()* som inträffar när 2.4.4-pX-versioner installeras.
* **ACSD-45049** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.4-p8 || >=2.4.5 &lt;2.4.6) - Korrigerar problemet där en kunds *[!UICONTROL Is required]*-attributinställning inte fungerar som den ska per webbplats i Admin.
* **ACSD-46938** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.6) - Korrigerar problemet med prestanda för DB-utlösare som återskapas under `setup:upgrade`.
...
* **ACSD-54887** (för Adobe Commerce och Magento Open Source >=2.4.4-p4 &lt;2.4.4-p9 || >=2.4.5-p3 &lt;2.4.5-p8 || >=2.4.6-p1 &lt;2.4.6-p6) - Korrigerar problemet där kundvagnen rensas efter kundsessionen har gått ut när [!UICONTROL Persistent Shopping Cart] är aktiverat.
* **ACSD-58141** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.8) - Korrigerar problemet där PHPSESSID genererar om POST-begäranden i butiksområdet för en inloggad kund om [!DNL L2 Redis cache] är aktiverat och kunden uppdateras från Admin.
...
* **ACSD-60441** (för Adobe Commerce och Magento Open Source 2.4.4-p9 || 2.4.5-p8 || 2.4.6-p6 || 2.4.7-p1) - Åtgärdar problemet med att uppdatera kunder via `V1/customers` [!DNL REST API] slutpunkten när integreringsåtkomsttoken som genererats från serverdelen används.
...
* **ACSD-58163** (för Adobe Commerce >=2.4.3 &lt;2.4.7) - Korrigerar problemet där en *[!UICONTROL Cart Price Rule]* inte tillämpar en rabatt för en gästkund från matchande *[!UICONTROL Customer Segment]*-kundvagn utan kupongkod.
* **ACSD-57045** (för Adobe Commerce >=2.4.5 &lt;2.4.7) - Korrigerar problemet där URL-omskrivningar orsakar oändliga sidupprepningar efter att *[!UICONTROL Website Root]* har avmarkerats från *[!UICONTROL Hierarchy]*.
* Uppdaterade patchar: ACSD-46815, ACSD-47027, ACSD-51683, ACSD-55031, ACSD-51819, ACSD-54965-v2

## v1.1.48 {#v1-1-48}

* **ACSD-55566** (för Adobe Commerce och Magento Open Source >=2.4.3 &lt;2.4.7) - Korrigerar problemet där `mergeCart`-mutationen misslyckas med ett *internt serverfel* i [!DNL GraphQL]-svaret när käll- och destinationskort som har samma paketobjekt sammanfogas.
* **ACSD-56546** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar problemet där konfigurerbara produkter och paketprodukter visas som **Out of Stock** i butiken när **visningen utanför produktkonfigurationen** är *Inaktiverad*.
...
* **ACSD-56741** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.7) - Korrigerar felmeddelandet &quot;*Försöker komma åt matrisförskjutning för värdet av typen null*&quot; som visas under `setup:upgrade` när databasen innehåller en anpassad [!DNL MySQL] -utlösare som inte är relaterad till indexeringsmekanismen och [!DNL MView].
* **ACSD-57315** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.7) - Korrigerar problemet när en ny transaktion skapas i [!DNL PayPal Payflow Pro] varje gång som knappen [!UICONTROL Fetch] klickas på skärmen **[!UICONTROL View transaction]** i Admin.
...
* **ACSD-57565** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.7) - Korrigerar problemet där kontrollpanelen **[!UICONTROL Order]** visar felaktig ordningsinformation tills tidsperioden har uppdaterats. På kontrollpanelen visas nu korrekt orderstatistik vid den första inläsningen.
* **ACSD-57854** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.7) - Korrigerar problemet när [!DNL GraphQL]-begäranden returnerar inaktiverade kategorier i kategoriaggregeringarna.
...
* **ACSD-57074** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.7) - Korrigerar problemet där det anpassade attributet *Yes/No* med `attrbute_code` som börjar med `price_` inte fungerar korrekt med indexering, och produkter med sådana attribut är inte tillgängliga på frontsidan.
...
* **ACSD-56842** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.7) - Korrigerar problemet där de fördröjda proxyfabrikerna och de fördröjda proxyfabrikerna saknas efter att ha kört `setup:di:compile`.
...
* **ACSD-56415** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.7) - Korrigerar problemet där prestanda för partiell prisindexering försämras på grund av en `DELETE`-fråga när databasen har många partiella prisdata att indexera.
...
* **ACSD-53925** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.7) - Korrigerar problemet där administratören inte kan spara CMS-blocket med produktkarusell när `catalog_product_price` dimensions-mode är inställt på *website*.
* **ACSD-52714** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.7) - Korrigerar problemet där datumfiltret inte fungerar i administratörsrutnätet när datumformatet är *Y-m-d*.
...
* **ACSD-54983** (för Adobe Commerce >=2.4.2 &lt;2.4.7) - Korrigerar ett fel där det inte går att hämta företagsanvändaren UID med GraphQL-begäran när användarstatusen är *[!UICONTROL Inactive]*.
* **ACSD-53309** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.7) - Korrigerar problemet där moms inte tillämpas fullt ut i *[!UICONTROL Regular Price]*-etiketten när det anpassningsbara alternativet har valts.
* **ACSD-55305** (för Adobe Commerce >=2.4.4 &lt;2.4.7) - Korrigerar felet där popup-fönstret *[!UICONTROL Edit Company User]* på **[!UICONTROL myAccount]** > **[!UICONTROL Company Structure]**-sidan låser sig med en inläsare på skärmen.
...
* **ACSD-55112** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.7) - Korrigerar problemet där knappen *[!UICONTROL Submit review]* kan klickas flera gånger utan [!DNL Google reCAPTCHA v3]-validering.
* **ACSD-54264** (för Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7) - Korrigerar problemet där felmeddelandet *&quot;Du kan inte uppdatera begärda attribut. Rad-ID: store_id* visas när en kund försöker checka ut med en överlåtbar offert från en annan butiksvy.
...
* **ACSD-55628** (för Adobe Commerce >=2.4.4-p2 &lt; 2.4.5 || >=2.4.5-p1 &lt; 2.4.6) - Korrigerar överföring av en fil i företagsregistreringsformuläret och ersättning av en fil för ett kundattribut på butiken.
...
* **ACSD-54040** (för Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6) - Korrigerar problemet där fältet *[!UICONTROL Created]* är tomt i ordningsinformation när B2B-moduler är aktiverade.
...
* **ACSD-54989** (för Adobe Commerce >=2.4.4-p5 &lt;2.4.5 || >=2.4.5-p4 &lt;2.4.6 || >=2.4.6-p2 &lt;2.4.7) - Korrigerar problemet där en företagsadministratör inte kan göra en beställning när *[!UICONTROL Enable Purchase Orders]* är inställt på *[!UICONTROL Yes]* och *[!UICONTROL Purchase Order]* är inställt på *[!UICONTROL No]*.
...
* **ACSD-55031** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.6) - Korrigerar *Typen &quot;mixed&quot; kan inte vara null* vid kompilering.
* **ACSD-54961** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.7) - Korrigerar problemet där en begränsad administratörsanvändare inte kan uppdatera statusen *Product Review*.
...
* **ACSD-53583** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Förbättrar prestanda för partiell omindexering för indexerare för *Kategoriprodukter* och *produktkategorier*.
...
* **ACSD-54776** (för Adobe Commerce >=2.4.5 &lt;2.4.7) - Korrigerar problemet där avmarkerade *[!UICONTROL Use Default Value]* och icke-standardvärden för produktfält inte sparas för den andra webbplats-, butiks- och butiksvyn.
* **ACSD-53998** (för Adobe Commerce och Magento Open Source >=2.4.4-p2 &lt;2.4.5 || >=2.4.5-p1 &lt;2.4.7) - Korrigerar problemet där ett **[!UICONTROL Dynamic Block]** baserat på ett **[!UICONTROL Customer Segment]** inte fungerar korrekt efter logga ut från ett kundkonto.
...
* **ACSD-54018** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.4.6) - Korrigerar prestandaproblemen med widgeten [!UICONTROL Product List] som använder ett icke-globalt attribut i widgetvillkoret.
...
* **ACSD-47669** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.6) - Korrigeringar *Överträdelse av integritetsbegränsning: 1452 Det går inte att lägga till eller uppdatera en underordnad rad: ett sekundärnyckelbegränsningsfel misslyckas* vid import av produkter CSV.
...
* **ACSD-53824** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar problemet där migreringsdatapappen `UpdateMultiselectAttributesBackendTypes` överskrider storleksgränsen för databastransaktion under `setup:upgrade`.

## v1.1.37 {#v1-1-37}

* **ACSD-52613** (för Adobe Commerce och Magento Open Source >=2.4.6 &lt;2.4.7) - Korrigerar problemet där cacher och index uppdateras även om inga uppdateringar görs av `Inventory_source`-objekt av REST API.
...
* **ACSD-53795** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.7) - Korrigerar problemet med ogiltig datatyp i kronjobbet `indexer_update_all_views`.
...
* **ACSD-53750** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.7) - Korrigerar felet *Bruten pipe eller stängd anslutning* under flertrådig omindexering av `catalog_product_price`.
* **ACSD-49843** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.7) - Korrigerar problemet där länken vid produktnedladdning inte är tillgänglig efter att den beställda artikeln automatiskt fakturerats med onlinebetalningsmetoden med **[!UICONTROL Payment Action]** = &lt;ph id= Inställningen 242/> har aktiverats.**[!UICONTROL Sale]**
...
* **ACSD-50887** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.7) - Korrigerar problemet där produktattributegenskapen *[!UICONTROL Use in Search Results Layered Navigation]* kan anges till *Yes* utan alternativet *[!UICONTROL Use in search]* inställt på *Yes* .
* **ACSD-51846** (för Adobe Commerce och Magento Open Source >=2.4.3-p2 &lt;2.4.6) - Korrigerar *Internal Error* -problemet som inträffar eftersom inte alla nivåer av REST API-nyttolast valideras.
...
* **ACSD-52095** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.4.6) - Korrigerar problemet där `manage_stock`-värdet felaktigt angavs till 0 i CSV-filen efter produktexporten.
...
* **ACSD-51890** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.7) - Korrigerar problemet där knappen [!UICONTROL Submit review] kan klickas flera gånger utan [!DNL Google reCAPTCHA] v3-validering.
* **ACSD-51984** (för Adobe Commerce >=2.4.5 &lt;2.4.7) - Korrigerar problemet där avmarkerade *[!UICONTROL Use Default Value]*- och *[!UICONTROL non-default product field]*-värden inte sparas för den andra webbplatsen, butiksvyn och butiksvyn.
...
* **ACSD-52786** (för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.6) - Korrigerar problemet där ett katalogregelvillkor *SKU är* gäller för alla produkter som börjar med angiven SKU.
...
* **ACSD-50813** (för Adobe Commerce >=2.4.5 &lt;2.4.7) - Korrigerar problemet där Admin inte kunde lägga till paketerade produkter som innehöll ett snedstreck i SKU med funktionen [!UICONTROL Add Products by SKU] i administratörsordningen.
...
* **ACSD-51857** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.7) - Korrigerar problemet där långsamma prestanda för `aggregate_sales_report_bestsellers_data` cron-rapporten för stora sales_order- och `sales_order_item`-databastabeller berodde på hur huvuddatafrågan skrevs.
...
* **ACSD-52148** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.4) - Korrigerar problemet där administratörsinloggningen för [!DNL Google V3 reCAPTCHA] ibland misslyckas.
* **ACSD-51431** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.4.7) - Korrigerar problemet där en indexerarstatus är *working* även om det inte finns några nya poster i ändringsloggen.
...
* **ACSD-49628** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.7) - Korrigerar problemet där flera fel i [!UICONTROL Page Builder's] förhindrar att administratören sparar en produkt utan innehållsbehörighet.
...
* **ACSD-51041** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.4.0 || >=2.4.1 &lt;2.4.6) - Förbättrar prisindexerarens prestanda.
...
* **ACSD-50345** (för Adobe Commerce och Magento Open Source >=2.4.3 &lt;2.4.4 || >=2.4.4-p1 &lt;2.4.6) - Korrigerar problemet där Recaptcha v2 inte läses in igen efter att en misslyckad betalning har skickats.
...
* **ACSD-51036** (för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.5) - Korrigerar problemet där konkurrensvillkoren under samtidiga REST API-anrop resulterar i att leveransstatusinformationen skrivs över i tabellen [!UICONTROL Items Ordered].
...
* **ACSD-49877** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.4.7) - Korrigerar problemet där automatisk uppspelning av video inte fungerar på mobilen [!DNL Safari] när videon är länkad direkt till en fjärrvideofil och inte en direktuppspelningstjänst.
...
* **ACSD-49370** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.4.7) - Korrigerar problemet där produktattributet *Date Time* har typen *FilterMatchTypeInput* i GraphQL-schemat.
...
* **ACSD-48216** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) - Korrigerar problemet där [!UICONTROL AUTO_INCREMENT] i tabellen [!UICONTROL inventory_source_item] ökar i tabellen &lt;ph id=&#39;153 åtgärd &#39;/>.[!UICONTROL UPDATE]
* **ACSD-47908** (för Adobe Commerce och Magento Open Source >=2.3.7 &lt;2.3.8 || >=2.4.0 &lt;2.4.7) - Korrigerar felet &quot;Ett värde som är mindre än eller lika med 0 förväntas&quot; när du väljer källa och kvantitet i leveranssteget vid utcheckning.
...
* **ACSD-45168** (för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.6) - Korrigerar problemet där SEO-vänliga URL:er inte genereras för produkter som har *url_key* -attribut åsidosatta på butiksvisningsnivå.
...
* **ACSD-46770** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.6) - Korrigerar problemet där e-postmeddelanden med administratörsorder skickas även när *bekräftelsen på e-postorder* inte är markerad.
...
* **ACSD-47920** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.6) - Korrigerar problemet där beställningar kan göras via Rest API som gästanvändare även när *Allow Gest Checkout* är inaktiverat.
...
* **ACSD-47444** (för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.3) - Korrigerar _Försök att få åtkomst till matrisförskjutning för typbool_ -fel vid åtkomst till vissa kategorisökvägar för kända produkter i PHP 7.4.
...
* **ACSD-46520** (*för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.5*) - Korrigerar det problem där en användare får en felaktig orderstatus när de återbetalas med butikskrediten.
...
* **ACSD-46815** (*för Adobe Commerce och Magento Open Source >=2.4.5 &lt;2.4.6*) - Korrigerar problemet där begäran om kategoriträd är begränsad till 20 kategorier.
...
* **MDVA-42768-V2** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.3*) - Korrigerar problemet där den konfigurerbara produkten visar det reguljära priset som *0* när *Visa ej i lager* är *Ja*.
* Uppdaterade patchar: MDVA-44562, ACSD-46213, MDVA-41305, MDVA-38346, MDVA-13203.
* Borttagen patch: MDVA-42768.

## v1.1.19 {#v1-1-19}

* **ACSD-46213** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.3*) - Korrigerar problemet där begäran om kategoriträd är begränsad till 20 kategorier.
* **ACSD-45781** (*för Adobe Commerce och Magento Open Source >=2.4.1 &lt;2.4.2*) - Korrigerar det problem där sökfältet framför i butiken inte visas på mobilen.
...
* **ACSD-46404** (*för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.5*) - Korrigerar det problem där en administratörsanvändare inte kan logga in efter uppgradering till 2.4.4.
...
* **ACSD-46146** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.6*) - Korrigerar problemet där två orderbekräftelsemeddelanden skickas efter att en beställning har gjorts från administratören.
...
* **ACSD-45241** (*för Adobe Commerce och Magento Open Source >=2.3.5 &lt;2.4.4*) - Korrigerar problemet där lagerkvantiteten för en virtuell produkt är felberäknad efter att en kreditnota har skapats.
...
* **MDVA-42807** (*för Adobe Commerce och Magento Open Source >=2.3.1 &lt;2.4.6*) - Korrigerar problemet där den anpassade valutatecknet inte visas på butikssidan.
...
* **MDVA-44562** (*för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.2-p2*) - Korrigerar problemet där det icke-förvalda arkiv-ID:t för offertobjekt åsidosätts av standardarkiv-ID:t, trots GraphQL-begäran som kommer från icke-standardbutiksvyn.
...
* **MDVA-44044** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.2-p2*) - Korrigerar problemet där en produkt inte visas på kategorisidan när den har tilldelats till en ny webbplats.
* **MDVA-42509** (*för Adobe Commerce och Magento Open Source >=2.3.3 &lt;2.4.4*) - Korrigerar problemet där en CSV inte kunde överföras för en snabborder som resulterade i ett *Det gick inte att skicka cookien* -fel.
...
* **MDVA-44887** (*för Adobe Commerce och Magento Open Source >=2.4.4 &lt;2.4.5*) - Korrigerar *Ohanterat SyntaxError: Oväntat token &#39;const&#39;*-fel på Admin-panelen.
...
* **MDVA-43983** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.5*) - Korrigerar problemet där produkter *inte visas individuellt* fortfarande visas i resultat för katalogavancerad sökning.
* **MDVA-44100** (*för Adobe Commerce och Magento Open Source >=2.4.3 &lt;2.4.5*) - Korrigerar problemet där alla FPT-filer har tilldelats till den sista produkten i kundvagnen och återställs för andra produkter.
...
* **MDVA-39605** (*för Adobe Commerce och Magento Open Source >=2.3.4 &lt;2.4.5*) - Korrigerar problemet där Redis cache TTL (utgångsdatum) har fel värde.
...
* **MDVA-43824** (*för Adobe Commerce och Magento Open Source >=2.3.6 &lt;=2.3.7-p3 || >=2.4.1*) - Korrigerar problemet där ett fel uppstår vid annullering av order med rabatt.
...
* **MDVA-42046** (*för Adobe Commerce och Magento Open Source >=2.3.4 &lt;=2.3.5-p2 || >=2.4.0 &lt;2.4.5*) - Åtgärdar problemet där ett felaktigt värde tilldelas ett produktattribut med ett datumindatafält när en produkt uppdateras.
...
* **MDVA-42283** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.5*) - Korrigerar problemet där datumet-time-formatet i det franska språkets administratörsorderrutnät är ogiltigt.
...
* **MDVA-39993** (*för Adobe Commerce och Magento Open Source >=2.3.5 &lt;=2.3.7-p2 || >=2.4.0 &lt;2.4.4*) - Korrigerar problemet där lagerändringar som gjorts med API inte återspeglas på produktsidan i klientsidan.
...
* **MDVA-39546** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.5*) - Korrigerar problemet där startdatumet för mellanlagringsuppdateringen kunde anges till ett tidigare datum än det aktuella datumet under redigeringen.
* **MDVA-42950** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.5*) - Korrigerar problemet där videor inte spelas upp på produktsidan.
* **MDVA-42689** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.4*) - Korrigerar problemet där Adobe Commerce orsakar ett *integritetsbegränsningsfel* vid uppdatering av produktkategorier under import.
* **MDVA-41229** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.5*) - Korrigerar problemet där bilder som är tillgängliga på baksidan inte visas på klientsidan efter konfigureringsbar produktimport.
* **MDVA-43731** (*för Adobe Commerce och Magento Open Source >=2.4.3 &lt;2.4.4*) - Korrigerar problemet där *Sök efter synonymer* inte längre fungerar när värdet läggs till i *Minimivillkor att matcha*.
...
* **MDVA-42269** (*för Adobe Commerce och Magento Open Source >=2.4.3-p1 &lt;2.4.5*) - Korrigerar problemet där en administratörsanvändare inte kan logga in i administratören på grund av felet *TypeError: strtotime() förväntar att parameter 1 ska vara sträng, null given* .
...
* **MDVA-42237** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.5*) - Korrigerar problemet där ett konfigurerbart specialpris för en produkt inte uppdateras efter ändringar i dess underproduktspris.
...
* **MDVA-40175** (*för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.3*) - Korrigerar problemet där *Klicka för att ändra leveransmetod* inte visar alternativknappar för att välja leveransmetoder i Admin under omsorteringen.
* **MDVA-42768** (*för Adobe Commerce och Magento Open Source >=2.3.4 &lt;2.4.5*) - Korrigerar problemet där den konfigurerbara produkten visar det normala priset 0 när *Visa slut-på-stock* är Ja.
...
* **MDVA-38626** (*för Adobe Commerce och Magento Open Source >=2.3.3 &lt;2.4.5*) - Korrigerar problemet där administratörsanvändaren inte kan lägga en order i backend med betalningen [!DNL PayPal Payflow Pro].
...
* **MDVA-41215** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.4*) - Korrigerar problemet där användare får 500-felet efter att ha angett *image-messages* -cookien om den redan finns, men det finns inga nya meddelanden.
...
* **MDVA-42326** (*för Adobe Commerce och Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - Korrigerar problemet där kunderna får ett fel i utcheckningen efter en sessionstimeout även om den beständiga kundvagnen är aktiverad.
...
* **MDVA-38393** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.4*) - Korrigerar problemet där katalogregler slutar fungera för en konfigurerbar produkt om dess enkla produkt får ett nytt namn.
...
* **MDVA-39711** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.3*) - Korrigerar det problem där administratörsanvändare inte kan komma åt kundens rutnät efter att webbplatsen har tagits bort.
...
* **MDVA-41631** (*för Adobe Commerce och Magento Open Source >=2.4.1 &lt;2.4.4*) - Korrigerar det problem där användare får ett fel när de försöker hämta orderinformation utan ett valfritt *telefonvärde* via GraphQL.
...
* **MDVA-40550** (*för Adobe Commerce och Magento Open Source >=2.3.5 &lt;2.4.4*) - Korrigerar problemet med saknade produkter på klientsidan under omindexering.
...
* **MDVA-41399** (*för Adobe Commerce och Magento Open Source >=2.3.3 &lt;2.4.2*) - Korrigerar problemet där administratörsanvändare inte kan komma åt sidan *Hantera kundvagn* om en kund lägger till en produkt i önskelistan.
...
* **MDVA-39031** (*för Adobe Commerce och Magento Open Source >=2.4.1 &lt;2.4.4*) - Korrigerar problemet där det går att lägga till en produkt i kundvagnen via GraphQL även om den inte har tilldelats till målwebbplatsen.
* **MDVA-41597** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.4*) - Korrigerar problemet där användare får ett fel när de lägger till mer än en konfigurerbar produkt i vagnen med GraphQL.
...
* **MDVA-32776** (*för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.2*) - Korrigerar problemet där lagerstatus inte uppdateras när en order läggs men inte skickas.
...
* **MDVA-39229** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.4*) - Korrigerar problemet som orsakar följande fel när katalogregeluppdateringen uppdaterar starttid: *Cron Job staging_synchronize_entities_period har ett fel: Den aktiva uppdateringen kan inte utföras tas bort.*
* **MDVA-40619** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;2.4.4*) - Korrigerar problemet där ändringar i CMS sidhierarki orsakar ett 500-fel vid infogad redigering på en CMS-sida.
* **MDVA-41061** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.3*) - Korrigerar det problem där Stock-statusen återställs till försäljningsbar när en produkt sparas från administratören.
...
* **MDVA-40101** (*för Adobe Commerce och Magento Open Source >=2.3.2 &lt;2.4.0*) - Korrigerar problemet där objekt inte tas bort från minivagnen efter en genomförd orderplacering med hjälp av [!DNL PayPal Express] Kassa.
...
* **MDVA-37725** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;=2.4.3-p1*) - Korrigerar problemet där asynkrona e-postmeddelanden om beställningar som skickas från icke-standardwebbplatser innehåller URL:er för logotyp från standardwebbplatsen.
* **MDVA-39482** (*för Adobe Commerce och Magento Open Source >=2.3.6 &lt;=2.3.7-p2 || >=2.4.1 &lt;2.4.4*) - Korrigerar problemet där en produkt slutar att vara i lager om den importeras med kvantiteten &quot;0&quot; när restorder är aktiverade.
* **MDVA-40435** (*för Adobe Commerce och Magento Open Source >=2.3.4 &lt;2.4.4*) - Korrigerar problemet med en felaktig rabatt på paketprodukter med dynamiska priser när de tillämpas via GraphQL.
...
* **MDVA-29400** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;=2.3.7-p1 || >=2.4.0 &lt;=2.4.0-p1*) - Korrigerar problemet med dubblettorder som har placerats med [!DNL PayPal Express Checkout].
...
* **MDVA-37234** (*för Adobe Commerce och Magento Open Source >=2.3.5 &lt;2.4.0 || >=2.4.1 &lt;=2.4.2-p2*) - Korrigerar problemet där ett objekt i kundvagnen skapas flera gånger (parallell begäran) för samma SKU för samma kundvagn-ID.
...
* **MDVA-36309** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;=2.4.2-p2*) - Korrigerar problemet där produktsökning med attribut är långsam i administratörsrutnätet.
...
* **MDVA-39195** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;=2.4.2-p2*) - Korrigerar problemet där knappen *Lägg till i kundvagnen* var inaktiv på kategorisidan när omdirigering till kundvagnen aktiverades.
* **MDVA-37115** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;=2.4.2-p2*) - Korrigerar problemet där ett onödigt *Endast 0 kvar*-meddelande visas på den konfigurerbara produktsidan.
* **MDVA-39521** (*för Adobe Commerce och Magento Open Source >=2.4.0 &lt;2.4.4*) - Korrigerar problemet där användaren inte kan ange leveransadresser i vagnen med ett tomt telefonnummer via GraphQL.
...
* **MDVA-39043** (*för Adobe Commerce och Magento Open Source >=2.3.4 &lt;=2.4.3*) - Korrigerar problemet där administratörsanvändaren med begränsad åtkomst får ett fel när han/hon försöker lägga till *Products*-widgeten på CMS-sidan.
...
* **MDVA-38852** (*för Adobe Commerce och Magento Open Source >=2.3.0 &lt;=2.3.5-p2*) - Korrigerar problemet där kataloginventeringen genom design låser tabeller för uppdateringar som avsevärt försämrar prestanda vid flera parallella order.
* **MDVA-39986** (*för Adobe Commerce och Magento Open Source >=2.4.1 &lt;2.4.3*) - Korrigerar problemet där användaren inte kan göra en beställning i Admin i MacOS med webbläsaren Safari.
* **MDVA-38447** (*för Adobe Commerce och Magento Open Source >=2.4.2 &lt;2.4.4*) - Korrigerar två problem: där *Inte synliga individuellt* konfigurerbara underordnade produkter returneras i GraphQL-svar och optimerar MySQL-frågan för GraphQL-produktfråga med kategorifilter.
...
* **MDVA-34948** (*för Adobe Commerce och Magento Open Source >=2.3.6 &lt;=2.3.6-p1 || >=2.4.0 &lt;=2.4.0-p1*) - Åtgärdar problemet med långvariga frågor, t.ex. &lt;ph id=&#39;155 &#39;/>.`GET_LOCK`
...
* **MDVA-34680** (*för Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;2.4.3*) - Korrigerar problemet där kundkontots skapade tid inte filtreras korrekt i kundens rutnät.
* **MDVA-37068** (*för Adobe Commerce >=2.3.1 &lt;2.4.4*) - Korrigerar problemet där fel skattesats visas när kundvagnen bara har virtuella produkter.
* **MDVA-38608** (*för Adobe Commerce >=2.3.0 &lt;2.4.3*) - Korrigerar problemet där temporära tabeller inte tas bort när omindexeringen inte slutförs.
* **MDVA-38308** (*för Adobe Commerce >=2.3.5 &lt;=2.3.6-p1 || >=2.4.0 &lt;=2.4.1-p1 || >=2.4.2 &lt;2.4.4*) - Korrigerar problemen med att lägga till &lt;ph id= &quot;136&quot;/> videor till produkterna.[!DNL Vimeo]
...
* **MDVA-37082** (*för Adobe Commerce >=2.3.0 &lt;2.4.3*) - Korrigerar problemet när den anpassade versionen av grupperade produkter sparas så att produkten inte finns i lager i förgrunden.
* **MDVA-36572** (*för Adobe Commerce >=2.3.5 &lt;2.4.3*) - Korrigerar problemet när kreditnotan inte längre orsakar dubbla produktreservationsuppdateringar i databasen.
...
* **MDVA-37874** (*för Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;=2.4.2-p1*) - Korrigerar problemet där *Fast rabattbelopp för hela vagnen&lt;id=&#39;140&#39;/> tillämpas felaktigt på en paketprodukt som innehåller mer än ett alternativ.*
...
* **MDVA-37225** (*för Adobe Commerce >=2.4.1 &lt;=2.4.2-p1*) - Korrigerar problemet där överföringsprocessen fastnade när snabborder skapades när det finns ett heltalsvärde i importerade SKU:er.
...
* **MDVA-36286** (*för Adobe Commerce >=2.3.6 &lt;=2.4.2-p1*) - Korrigerar problemet där förhandsvisningen av Page Builder-produktwidgeten bryts om samma SKU har en annan position i underkategorier.
* **MDVA-30186** (*för Adobe Commerce >=2.3.4 &lt;=2.3.5-p2, >=2.4.0 &lt;=2.4.0-p1, >=2.4.2 &lt;=2.4.2-p1*) - Korrigerar problemet där attributalternativen är sorterade som anges av alternativvärdet i stället för av antalet attributobjekt i GraphQL svar.
...
* **MDVA-36424** (*för Adobe Commerce >=2.3.0 &lt;=2.3.3-p1 || >=2.0.0 &lt;2.2.0*) - Korrigerar problemet där mediabilder som är kopplade till sidbyggarelement försvinner när innehållet redigeras upprepade gånger om serverdelens bas-URL är annorlunda från butikens bas-URL.
...
* **MDVA-11189** (*för Adobe Commerce >=2.3.0 &lt;2.3.5*) - Korrigerar problemet när en CSV-fil för uppdatering av produktlager har importerats och rader från tabellen `cataloginventory_stock` tas bort.
* **MDVA-26639** (*för Adobe Commerce >=2.3.3-p1 &lt;2.3.6*) - Korrigerar det problem där orderobjekten saknas i orderpostmeddelandet om en ny e-postmall för orderbekräftelse skapas.
...
* **MDVA-21095** (*för Adobe Commerce >=2.3.0 &lt;2.3.5*) - Korrigerar problemet när en fråga `INSERT INTO search_tmp` inte avslutas efter att massattributvärdet har uppdaterats.
* **MDVA-23845** (*för Adobe Commerce >=2.3.2-p2 &lt;2.3.5*) - Korrigerar problemet där e-postmallar inte kan förhandsgranskas när JavaScript minification har aktiverats.
* **MDVA-22026** (*för Adobe Commerce >=2.3.2 &lt;2.3.4*) - Korrigerar problemet där import av produkter från CSV-fil, inklusive bilder från externa URL:er, misslyckas.
* **MDVA-22383** (*för Adobe Commerce >=2.3.0 &lt;2.3.4*) - Korrigerar ett fel där det tar lång tid att spara en produkt och sidan går sönder.
* **MC-41359** (*för Adobe Commerce >=2.3.6-p1 &lt;2.3.7, >=2.4.2 &lt;2.4.3*) - Korrigerar problemet med felaktiga inställningar för SameSite-cookie.
...
* **MDVA-35982** (*för Adobe Commerce >=2.3.0 &lt;2.4.3*) - Korrigerar problemet där administratörsanvändare som är begränsade till en viss webbplats inte kan skapa en leverans för den order som finns på samma webbplats.
...
* **MDVA-35847** (*för Adobe Commerce >=2.4.1 &lt;2.4.3*) - Korrigerar B2B-problemet där företagsanvändare skapar fel om ett anpassat kundattribut används.
* **MDVA-31307** (*för Adobe Commerce >=2.4.0 &lt;2.4.2*) - Korrigerar problemet där det finns *Slut på minne* i vissa kategorier på grund av problem med dynamisk CSP-vitlista för cachelagrade block.

## v1.0.18 {#v1-0-18}

* **MDVA-32655** (*för Adobe Commerce >=2.3.0 &lt;2.4.3*) - Korrigerar den felaktiga *in progress* -meddelandestatusen till rätt *complete* -meddelande för konsumenten `quoteItemCleaner` efter att flera produkter har tagits bort.
* **MDVA-34102** (*för Adobe Commerce >=2.3.0 &lt;2.4.3*) - Korrigerar att standardlagret är noll för inaktiverade produkter i produktstödrastret och Redigera produktsidor i administratörsområdet.
* **MDVA-35286** (*för Adobe Commerce >=2.4.0 &lt;2.4.2*) - Korrigerar problemet där det uppstår ett fel om en kund har paketerat produkter i kundvagnen och byter från utcheckning av flera adresser till utcheckning av en sida.
...
* **MDVA-35197** (*för Adobe Commerce >=2.3.5 &lt;2.4.0*) - Korrigerar problemet där det uppstod ett fel när du lade till i kundvagnen med GraphQL om tidigare tillagda produkter inte finns i lager.
* **MDVA-34850** (*för Adobe Commerce >=2.3.1 &lt;2.4.0*) - Korrigerar problemet där optioner som inte finns på plats för en konfigurerbar produkt inte visas i stället för att visas som genomstruken.
...
* **MDVA-35092** (*för Adobe Commerce >=2.3.5 &lt;2.4.3*) - Korrigerar problemet där användare inte kan lägga till [!DNL Vimeo] videor på grund av det inaktuella [!DNL Vimeo] API:t.

## v1.0.16 {#v1-0-16}

* **MDVA-33453** (*för Adobe Commerce >=2.3.6 &lt;2.4.3*) - Korrigerar problemet där Page Builder-produkters innehållstypförhandsvisning bryts om matchande produkter har olika priser för olika webbplatser.
...
* **MDVA-34192** (*för Adobe Commerce >=2.3.4 &lt;2.4.3*) - Korrigerar problemet där det inte går att ändra/ange kundens födelsedatum i dd/mm/åååå-format.
...
* **MDVA-33559** (*för Adobe Commerce >=2.3.0 &lt;2.4.3*) - Korrigerar problemet med att [!DNL PayPal Payflow Pro] betalning inte kunde betalas med formatfel för omdirigeringsparameterlista.
...
* **MDVA-33632** (*för Adobe Commerce >=2.3.0 &lt;2.3.7*) - Tillhandahåller en korrigering där undantagsmeddelandet *This product is out of stock* visas nu för en admin-användare som försöker beställa om en produkt från Stock.
...
* **MDVA-33894** (*för Adobe Commerce >=2.4.0 &lt;2.4.1 med B2B-tillägg*) - Korrigerar flera problem med snabbordningsfunktionen, inklusive tillägg och borttagning av flera produkter och SKU-skiftlägeskänslighet.
* **MDVA-27664** (*för Adobe Commerce >=2.3.4 &lt;2.3.6*) - Korrigerar problemet i kundregistreringsformuläret som orsakar ett fel: *ERROR - Födelsedatumet får inte vara senare än idag.*
* **MDVA-33970** (*för Adobe Commerce >=2.3.4 &lt;2.4.2*) - Korrigerar problemet där det finns fel valutatecken i kreditnotan när prisattributets omfång är inställt på webbplatsen.
...
* **MDVA-28511** (*för Adobe Commerce >=2.3.5 &lt;2.3.6*) - Korrigerar problemet där det inte går att slutföra [!DNL PayPal] utcheckningen om namnfältet innehåller vissa tecken (t.ex. versaler).
...
* **MDVA-24201** (*för Adobe Commerce >=2.3.0 &lt;2.3.5*) - Korrigerar problemet där priserna inte återspeglar den schemalagda kundprisregeln förrän du indexerar om manuellt.
* **MDVA-32694** (*för Adobe Commerce >=2.3.0 &lt;2.3.6 || >= 2.4.0 &lt;2.4.2*) - Korrigerar problemet där en administratörsanvändare inte kan lägga till en produkt i en överlåtbar offert om den är relaterad till ett annat lager.
...
* **MDVA-20376** (*för Adobe Commerce >=2.3.2 &lt;2.3.4*) - Korrigerar problemet med felet *Ingen sådan entitet med customerId = 1* i `exception.log` för inloggade kunder efter orderplacering.
...
* **MDVA-13203** (*för Adobe Commerce >=2.3.0 &lt;2.4.2*) - Korrigerar problemet där felet *Integritetsbegränsning i Search_tmp_ table* visas efter en fullständig indexering.
...
* **MDVA-32545** (*för Adobe Commerce >=2.3.0 &lt;2.4.2*) - Korrigerar problemet där fakturor inte skickas automatiskt när du skapar order från administratören.
...
* **MDVA-30845** (*för Adobe Commerce >=2.3.5 &lt;2.3.7*) - Korrigerar ett fel där *Tyvärr visas inga citattecken för den här beställningen just nu* när det inte går att ansluta till UPS XML/USPS/DHL, och ingen annan leveransmetod är tillgängligt.
...
* **MDVA-32449** (*för Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0 || >=2.4.1 &lt;2.4.2 med B2B-tillägg*) - Korrigerar problemet där orderhistoriken läses in mycket långsamt eller inte alls.
...
* **MDVA-31363** (*för Adobe Commerce >=2.3.2 &lt;2.4.2*) - Korrigerar problemet där kundvagnsregeln med kupong inte gäller via GraphQL när *Fast beloppsrabatt för hela kundvagnen* används.
* **MDVA-30889** (*för Adobe Commerce >=2.3.0 &lt;2.4.2*) - Korrigerar problemet när ett fel inträffar efter att ett paket med virtuella och enkla produkter har anropats som alternativ.
* **MDVA-31791** (*för Adobe Commerce >=2.3.4 &lt;2.3.5*) - Förbättrar prestandan för produktsidan om målregler eller länkade produkter används.
...
* **MDVA-28651** (*för Adobe Commerce >=2.3.0 &lt;2.3.6 || >2.4.0 med B2B-tillägg*) - Korrigerar problemet där det uppstår prestandaproblem vid inläsning av överlåtbara offerter via REST API.

## v1.0.8 {#v1-0-8}

* **MDVA-31242** (*för Adobe Commerce >=2.3.0 &lt;2.4.1 med B2B-tillägg*) - Korrigerar problemet där fel valutasymbol visas i rutnätet för kreditnotor.
...
* **MDVA-30837** (*för Adobe Commerce >=2.3.1 &lt;2.3.5*) - En konfigurationsinställning har lagts till *Inkludera moms i Belopp: Ja/Nej* i konfigurationen för kostnadsfri leverans. När *Inkludera skatt till belopp* är inställt på *Ja* beräknas minimiorderbelopp som Delsumma + moms. När *Inkludera moms till belopp* är inställt på *Nej* beräknas minimiorderbelopp som delsumma
* **MDVA-25028** (*för Adobe Commerce >=2.3.2 &lt;2.3.3 || >=2.3.5 &lt;2.3.6*) - Korrigerar problemet när beställningar som har placerats med [!DNL PayPal Payflow Pro] inte är inställda på misstänkt bedrägeri Bedrägerifilter aktiveras.
...
* **MDVA-30945** (*för Adobe Commerce >=2.3.0 &lt;2.4.2*) - Korrigerar problemet där du får ett allvarligt felmeddelande när du uppdaterar kundvagnen `Call to a member function getValue() on null in module-configurable-product CartItemProcessor.php`.

## v1.0.6 {#v1-0-6}

* **MDVA-28993** (*för Adobe Commerce >=2.3.4 &lt;2.4.0*) - Implementerar funktionen *Minimum ska matcha* och partiell sökning för [!DNL Elasticsearch] engine. Löser problem med bindestreck i sökfrågor.
...
* **MDVA-30599** (*för Adobe Commerce >=2.3.4 &lt;=2.4.0*) - Korrigerar problemet där gästcitat som skapats med API felaktigt markeras som offerter för inloggade kunder.
...
* **MDVA-30565** (*för Adobe Commerce >=2.3.2 &lt;=2.3.3-p1*) - Korrigerar problemet där *ingen sådan entitet med cartid = 0*-fel visas för gästkund vid utcheckning på plats om en beständig kundvagn är aktiverad.
...
* **MDVA-28202** (*för Adobe Commerce >=2.3.4 &lt;=2.4.2*) - Korrigerar problemet där Layered Navigation inte filtrerar konfigurerbara produkter korrekt när MSI används.
...
* **MDVA-28191** (*för Adobe Commerce >=2.3.3 &lt;2.4.2*) - Korrigerar problemet där inga betalningsmetoder läses in när beställningen skapas via Admin.
* **MDVA-29959** (*för Adobe Commerce >=2.3.0 &lt;=2.3.3-p1 med B2B-tillägg*) - Korrigerar problemet där begränsad admin-användare med *Företag* behörighet inte får ta bort företagskonto.
* **MDVA-30265** (*för Adobe Commerce >=2.3.3 &lt;2.4.2*) - Korrigerar problemet där försändelsespårningslänken slutar fungera när fakturan har skapats.
...
* **MDVA-30593** (*för Adobe Commerce >=2.3.0 &lt;2.3.4*) - Korrigerar problemet där citattecken som har upphört enligt inställningen för offertlivstid inte rensas.
...
* **MDVA-28661** (*för Adobe Commerce >=2.3.0 &lt;2.4.2 med B2B-tillägg*) - Korrigerar problemet där ett fel uppstår i företagsanvändarkontoavsnittet när företagsadministratören har ändrats.
...
* **MDVA-28656** (*för Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.2*) - Korrigerar problemet där en beställning utan betalningsinformation (t.ex. med 100 % rabatt) och en faktura skapades för orderstatus ändras till *Closed* i stället för Complete.
...
* **MDVA-29996** (*för Adobe Commerce >=2.3.3 &lt;2.4.2*) - Korrigerar problemet där kategorisidan inte cachas av helagrad sida när kategoribehörighet har aktiverats.
...
* **MDVA-30444** (*för Adobe Commerce >=2.3.0 &lt;2.4.1*) - Korrigerar problemet där ingen bekräftelse skickas via e-post för beställningar som gjorts med GraphQL.
...
* **MC-35984** (*för Adobe Commerce >=2.4.0 &lt;2.4.1*) - Korrigerar problemet där handlare inte kunde interagera med sidelement på retursidan efter att ha skapat en leveransetikett för en RMA (Return Merchandise Authorization).
...
* **MDVA-26694** (*för Adobe Commerce >=2.3.0 &lt;2.3.6 || 2.4.0*) - Korrigerar problemet med produkt- och katalogcacheminnen som förfaller dagligen, men som schemaläggs till en annan tidpunkt.
...
* **MDVA-29085** (*för Adobe Commerce >=2.3.0 &lt;=2.3.5-p1*) - Korrigerar B2B-problemet där inga nya e-postmeddelanden skickas ut om ett företag har skapats med API.
* **MDVA-29344** (*för Adobe Commerce >=2.3.5 &lt;=2.4.0-p1*) - Korrigerar problemet där Page Builder fastnar efter kopiering av text från ett rubrikelement till ett textelement.
